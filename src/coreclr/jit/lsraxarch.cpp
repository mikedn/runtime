...
int LinearScan::BuildNode(GenTree* tree)
{
    ...
    switch (tree->OperGet())
    {
        ...
        case GT_BOUNDS_CHECK:
            assert(dstCount == 0);
            srcCount = BuildOperandUses(tree->AsBoundsChk()->GetOp(0));
            srcCount += BuildOperandUses(tree->AsBoundsChk()->GetOp(1));
            break;
        ...
        case GT_STOREIND:
            ...
            {
                srcCount = BuildStoreInd(tree->AsStoreInd());
            }
            break;
        ...
        case GT_IND:
            srcCount = BuildLoadInd(tree->AsIndir());
            assert(dstCount == 1);
            break;
...
int LinearScan::BuildCmp(GenTreeOp* cmp)
{
    ...
    if (op1->IsCast() && op1->isContained())
    {
        srcCount += BuildOperandUses(op1->AsCast()->GetOp(0), op1Candidates);
    }
    else
    {
        srcCount += BuildOperandUses(op1, op1Candidates);
    ...
    if (op2->IsCast() && op2->isContained())
    {
        srcCount += BuildOperandUses(op2->AsCast()->GetOp(0), op2Candidates);
    }
    else
    {
        srcCount += BuildOperandUses(op2, op2Candidates);
