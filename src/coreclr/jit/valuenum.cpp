    ...
    else
    {
        ...
        if (valueType != defType)
        {
            ...
            else
            {
                ...
                valueVNP.SetLiberal(vnStore->VNForCast(valueVNP.GetLiberal(), defType));
                valueVNP.SetConservative(vnStore->VNForCast(valueVNP.GetConservative(), defType));
            }
        }
    }
...
void ValueNumbering::NumberLclUse(GenTreeLclUse* use)
{
    ...
    if (valSize != lclSize)
    {
        ...
        {
            vnp = {vnStore->VNForFunc(TYP_INT, VNOP_TRUNC, vnp.GetLiberal()),
                   vnStore->VNForFunc(TYP_INT, VNOP_TRUNC, vnp.GetConservative())};
        }
        else
        {
...
void ValueNumbering::NumberDivMod(GenTreeOp* node)
{
    ...
    if (GenTree* divisor = node->GetOp(1)->SkipComma()->IsIntCon())
    {
        if (UsesDivideByConstOptimized(node))
        {
...
bool ValueNumbering::UsesDivideByConstOptimized(GenTreeOp* div)
{
    ...
    ssize_t divisor = static_cast<ssize_t>(div->GetOp(1)->SkipComma()->AsIntCon()->GetValue());
    ...
    {
        if (type == TYP_INT ? isPow2(static_cast<uint32_t>(divisor & UINT32_MAX))
                            : isPow2(static_cast<uint64_t>(divisor)))
        {
            return true;
        }
    }
    ...
    else if (div->OperIs(GT_UDIV))
    {
        // If the divisor is greater or equal than 2^(N - 1) then the result is either 0 or 1
        if (((type == TYP_INT) && (static_cast<uint32_t>(divisor) > (UINT32_MAX / 2))) ||
            ((type == TYP_LONG) && (static_cast<uint64_t>(divisor) > (UINT64_MAX / 2))))
        {
            return true;
        }
    }
