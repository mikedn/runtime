...
void LinearScan::clearNextIntervalRef(RegNum reg, var_types regType)
{
...
LsraLocation LinearScan::GetNextFixedRef(RegNum reg, var_types regType) const
{
    LsraLocation loc = GetNextFixedRef(reg);

#ifdef TARGET_ARM
    if (regType == TYP_DOUBLE)
    {
        loc = Min(loc, GetNextFixedRef(REG_NEXT(reg)));
    }
#endif
...
void LinearScan::updateNextIntervalRef(RegNum reg, Interval* interval)
{
    assert((REG_FIRST <= reg) && (reg <= REG_LAST));
...
LsraLocation LinearScan::GetNextIntervalRef(RegNum reg, var_types regType) const
{
    LsraLocation loc = GetNextIntervalRef(reg);

#ifdef TARGET_ARM
    if (regType == TYP_DOUBLE)
    {
        loc = Min(loc, GetNextIntervalRef(REG_NEXT(reg)));
    }
#endif
...
void LinearScan::clearSpillCost(RegRecord* reg, var_types regType) const
{
    ...
    if (regType == TYP_DOUBLE)
    {
        reg->GetDoublePairNextReg()->spillCost = 0;
    }
#endif
}
...
void LinearScan::updateSpillCost(RegRecord* reg, Interval* interval) const
{
    ...
    if (interval->registerType == TYP_DOUBLE)
    {
        reg->GetDoublePairNextReg()->spillCost = cost;
    }
#endif
}
...
regMaskTP LinearScan::getConstrainedRegMask(regMaskTP regMaskActual, regMaskTP regMaskConstraint, unsigned minRegCount)
{
    ...
    return genCountBits(newMask) >= minRegCount ? newMask : regMaskActual;
}
...
bool LinearScan::conflictingFixedRegReference(regNumber regNum, RefPosition* refPosition) const
{
    ...
    const RegRecord* regRecord   = GetRegRecord(regNum);
...
void Interval::mergeRegisterPreferences(regMaskTP preferences)
{
...
void Interval::updateRegisterPreferences(regMaskTP preferences)
{
                                  ...
                                  RefPosition* refPosition DEBUG_ARG(RegisterScore* registerScore))
{
    ...
    if ((assignedInterval != currentInterval) && foundRegRecord->IsAssigned(ARM_ONLY(currentInterval->registerType)))
    {
...
bool LinearScan::canSpillReg(RegRecord* reg, LsraLocation refLocation) const
{
    assert(reg->assignedInterval != nullptr);

    if (RefPosition* recentAssignedRef = reg->assignedInterval->recentRefPosition)
    {
    ...
    assert(reg->assignedInterval->getLocalVar(compiler)->IsParam());
...
#ifdef TARGET_ARM
bool LinearScan::canSpillDoubleReg(RegRecord* reg, LsraLocation refLocation) const
{
    RegRecord* regs[]{reg, reg->GetDoublePairNextReg()};

    return ((regs[0]->assignedInterval == nullptr) || canSpillReg(regs[0], refLocation)) &&
           ((regs[1]->assignedInterval == nullptr) || canSpillReg(regs[1], refLocation));
}

void LinearScan::unassignDoublePhysReg(RegRecord* reg)
{
    RegRecord* regs[]{reg, reg->GetDoublePairNextReg()};

    if (regs[0]->assignedInterval == nullptr)
    {
        assert((regs[1]->assignedInterval != nullptr) && (regs[1]->assignedInterval->registerType == TYP_FLOAT));

        unassignPhysRegRecentRef(regs[1]);
    }
    else if (regs[0]->assignedInterval->registerType == TYP_DOUBLE)
    {
        unassignPhysRegRecentRef(regs[0]);
    }
    else
    {
        assert(regs[0]->assignedInterval->registerType == TYP_FLOAT);

        unassignPhysRegRecentRef(regs[0]);

        if (regs[1]->assignedInterval != nullptr)
        {
            assert(regs[1]->assignedInterval->registerType == TYP_FLOAT);
...
//    False - otherwise
//
bool LinearScan::isRefPositionActive(RefPosition* refPosition, LsraLocation refLocation) const
{
    return (refPosition->nodeLocation == refLocation) ||
           ((refPosition->nodeLocation + 1 == refLocation) && refPosition->delayRegFree);
}

// Determine if a register is a spill candidate for a given RefPosition.
bool LinearScan::isSpillCandidate(Interval* current, RefPosition* refPosition, RegRecord* reg) const
{
    // We shouldn't be calling this if we haven't already determined that the register
    // is not busy until the next kill.
    assert(!isRegBusy(reg->regNum, current->registerType));
    // We should already have determined that the register isn't actively in use.
    assert(!isRegInUse(reg->regNum, current->registerType));
    // We shouldn't be calling this if 'refPosition' is a fixed reference to this register.
    assert(!refPosition->isFixedRefOfRegMask(genRegMask(reg->regNum)));
    ...
    assert(!conflictingFixedRegReference(reg->regNum, refPosition));

#ifdef TARGET_ARM
    if (current->registerType == TYP_DOUBLE)
    {
        return canSpillDoubleReg(reg, refPosition->nodeLocation);
    }
#endif

    return canSpillReg(reg, refPosition->nodeLocation);
}
...
bool RegRecord::IsAssigned(ARM_ONLY(RegisterType newRegType)) const
{
    return (assignedInterval != nullptr)
#ifdef TARGET_ARM
           || ((newRegType == TYP_DOUBLE) && (GetDoublePairNextReg()->assignedInterval != nullptr))
#endif
        ;
}
...
void LinearScan::checkAndAssignInterval(RegRecord* regRec, Interval* interval)
{
    ...
    {
        RegRecord* regRecord2 = regRec->GetDoublePairNextReg();
        assignedInterval      = regRecord2->assignedInterval;

        if ((assignedInterval != nullptr) && (assignedInterval != interval))
        {
            if (assignedInterval->assignedReg == regRecord2)
            {
                assert(!assignedInterval->isActive);
                assignedInterval->physReg = REG_NA;
            }

            unassignPhysReg(regRecord2, nullptr);
        }
    }
#endif
...
//
// Assumptions:
//    "interval" must be a local interval, as tree temps are never split.
...
void LinearScan::setIntervalAsSpilled(Interval* interval)
{
    ...
    if (interval->isUpperVector)
    {
        ...
        if (!interval->isSpilled && interval->isActive && (recentRefPos != nullptr))
        {
            ...
            interval->isSpilled = true;
            RegRecord* reg      = GetRegRecord(interval->physReg);
            reg->spillCost      = getSpillWeight(reg);
        }
    }
#endif
    if (interval->isLocalVar)
...
void LinearScan::unassignPhysRegRecentRef(RegRecord* regRec)
{
...
void LinearScan::unassignPhysReg(RegRecord* regRec ARM_ARG(RegisterType newRegType))
{
        ...
        (regRecToUnassign->assignedInterval->registerType == TYP_DOUBLE))
    {
        ...
        {
            regRecToUnassign = regRec->GetDoublePairOtherReg();
        }
    }
    else
    {
        ...
        {
            anotherRegRec = regRecToUnassign->GetDoublePairNextReg();
        }
    }
#endif
...
//
void LinearScan::unassignPhysReg(RegRecord* const regRec, RefPosition* spillRefPosition)
{
    Interval* const assignedInterval = regRec->assignedInterval;
    assert(assignedInterval != nullptr);
    assert(spillRefPosition == nullptr || spillRefPosition->getInterval() == assignedInterval);
    regNumber const thisRegNum = regRec->regNum;
    ...
    if (assignedInterval->registerType == TYP_DOUBLE)
    {
        ...
        {
            anotherRegRec = regRec->GetDoublePairNextReg();
            doubleRegRec  = regRec;
        }
        else
        {
            regToUnassign = REG_PREV(thisRegNum);
            anotherRegRec = GetRegRecord(regToUnassign);
        ...
        clearSpillCost(GetRegRecord(regToUnassign), TYP_DOUBLE);
...
#endif // TARGET_ARM
    {
        ...
        clearSpillCost(regRec, assignedInterval->registerType);
    ...
    if (spill)
    {
        ...
        if (spillRefPosition->copyReg)
        {
            ...
            if (copyFromRegNum == thisRegNum &&
                GetRegRecord(copyRegNum)->assignedInterval == assignedInterval)
            {
                assert(copyRegNum != thisRegNum);
                assignedInterval->physReg = copyRegNum;
                assignedInterval->assignedReg = GetRegRecord(copyRegNum);
                return;
            }
        }
#endif // 0
    ...
    else if (canRestorePreviousInterval(regRec, assignedInterval))
    {
        ...
        {
            RegRecord* otherHalfReg        = regRec->GetDoublePairOtherReg();
            otherHalfReg->assignedInterval = regRec->assignedInterval;
            otherHalfReg->previousInterval = nullptr;
        }
#endif // TARGET_ARM
...
void LinearScan::spillGCRefs(RefPosition* killRefPosition)
{
    ...
    while (candidateRegs != RBM_NONE)
    {
        ...
        RegRecord* regRecord        = GetRegRecord(nextReg);
        ...
        if (!needsKill)
        {
            ...
            // we can have a LCL_LOAD node with a GC type, when the local itself is an integer type.
...
bool LinearScan::isSecondHalfReg(RegRecord* regRec, Interval* interval)
{
    ...
    return (assignedReg != nullptr) && (interval->registerType == TYP_DOUBLE) &&
           (regRec == GetRegRecord(assignedReg->regNum)->GetDoublePairNextReg());
}
#endif // TARGET_ARM
...
bool LinearScan::canRestorePreviousInterval(RegRecord* regRec, Interval* assignedInterval)
{
    ...
    {
        retVal = regRec->GetDoublePairOtherReg()->assignedInterval == nullptr;
    }
#endif
...
void LinearScan::processBlockStartLocations(BasicBlock* currentBlock)
{
    ...
    if (!enregisterLocalVars)
    {
        ...
        for (RegNum reg = REG_FIRST; reg <= REG_LAST; reg = REG_NEXT(reg))
        {
            ...
            clearNextIntervalRef(reg, regRecord->registerType());
            clearSpillCost(regRecord, regRecord->registerType());
    ...
    for (VarSetOps::Enumerator e(compiler, liveInVars); e.MoveNext();)
    {
        ...
        {
            // There are 3 cases where we will leave writethru locals on the stack:
        ...
        else if (interval->physReg != REG_NA)
        {
            ...
            if ((targetReg != REG_STK) || leaveOnStack)
            {
                ...
                {
                    interval->isActive = false;
                    unassignPhysReg(GetRegRecord(interval->physReg), nullptr);
                }
                else
                {
        ...
        if (targetReg != REG_STK)
        {
            RegRecord* targetRegRecord = GetRegRecord(targetReg);
            liveRegs |= getRegMask(targetReg, interval->registerType);

            if (!allocationPassComplete)
            {
                updateNextIntervalRef(targetReg, interval);
                updateSpillCost(targetRegRecord, interval);
            }
            ...
            {
#ifdef TARGET_ARM
                // If this is a DOUBLE interval, and the assigned interval is either null or is FLOAT,
                // we also need to unassign the other half of the register.
                // Note that if the assigned interval is DOUBLE, it will be unassigned below.
                if ((interval->registerType == TYP_DOUBLE) &&
                     ...
                     (targetRegRecord->assignedInterval->registerType == TYP_FLOAT)))
                {
                    ...
                    unassignIntervalBlockStart(targetRegRecord->GetDoublePairNextReg(),
                                               allocationPassComplete ? nullptr : inVarToRegMap);
                }

                // If this is a FLOAT interval, and the assigned interval was DOUBLE, we also
                // need to update the liveRegs to specify that the other half is not live anymore.
                // As mentioned above, for DOUBLE, the other half will be unassigned further below.
                if ((interval->registerType == TYP_FLOAT) &&
                    ((targetRegRecord->assignedInterval != nullptr) &&
                     (targetRegRecord->assignedInterval->registerType == TYP_DOUBLE)))
                {
                    // Use FLOAT to get the regmask of just the half reg.
                    liveRegs &= ~getRegMask(targetRegRecord->GetDoublePairOtherReg()->regNum, TYP_FLOAT);
                }
#endif // TARGET_ARM
    ...
    for (RegNum reg = REG_FIRST; reg <= REG_LAST; reg = REG_NEXT(reg))
    {
        ...
        if ((liveRegs & genRegMask(reg)) == RBM_NONE)
        {
            makeRegAvailable(reg, regRecord->registerType());
            Interval* assignedInterval = regRecord->assignedInterval;
            ...
            if (assignedInterval != nullptr)
            {
                ...
                if (!assignedInterval->isConstant && (assignedInterval->assignedReg == regRecord))
                {
                    assignedInterval->isActive = false;
...
//    at successor blocks during allocation time, but if locals  are spilled after a block has been
...
void LinearScan::freeRegister(RegRecord* physRegRecord)
{
    ...
    clearSpillCost(physRegRecord, physRegRecord->registerType());
...
// Free the registers in 'regsToFree'
void LinearScan::freeRegisters(regMaskTP regsToFree)
{
    ...
    while (regsToFree != RBM_NONE)
    {
        ...
        RegNum     nextReg   = genRegNumFromMask(nextRegBit);
        RegRecord* regRecord = GetRegRecord(nextReg);
...
void LinearScan::allocateRegisters()
{
    ...
    for (Interval& interval : intervals)
    {
        ...
        interval.isActive          = interval.isLocalVar && interval.getLocalVar(compiler)->IsRegParam() &&
    ...
    for (RegNum reg = REG_FIRST; reg <= REG_LAST; reg = REG_NEXT(reg))
    {
        ...
        updateNextFixedRef(regRecord, regRecord->firstRefPosition);
        ...
        if (Interval* interval = regRecord->assignedInterval)
        {
...
#endif
            {
                ...
                updateSpillCost(regRecord, interval);
    ...
    for (RefPosition& refPositionIterator : refPositions)
    {
        ...
        Referenceable* currentReferent     = currentRefPosition->referent;
        ...
        if (currentLocation > prevLocation)
        {
            ...
            if ((regsToFree | delayRegsToFree) != RBM_NONE)
            {
                ...
                for (RegNum reg = REG_FIRST; reg <= REG_LAST; reg = REG_NEXT(reg))
                {
                    ...
                    RegRecord* const regRecord        = GetRegRecord(reg);
                    Interval* const  assignedInterval = regRecord->assignedInterval;

                    if (assignedInterval != nullptr)
                    {
                        bool const isAssignedReg = (assignedInterval->physReg == reg);
                        ...
                        if (RefPosition* const recentRefPosition = assignedInterval->recentRefPosition)
                        {
                            ...
                            if (assignedInterval->isActive)
                            {
                                ...
                                // or it was placed there by the inVarToRegMap. In either case it must be a local.
                                ...
                                if (isAssignedReg)
                                {
                                    assert(GetNextIntervalRef(reg) == assignedInterval->getNextRefLocation());
                                    assert(!isRegAvailable(reg, assignedInterval->registerType));
                                    assert((recentRefPosition == nullptr) ||
                                           (regRecord->spillCost == getSpillWeight(regRecord)));
                                }
                                else
                                {
                                    assert((GetNextIntervalRef(reg) == MaxLocation) ||
                    ...
                    {
                        assert(isRegAvailable(reg, regRecord->registerType()));
                        assert(!isRegConstant(reg, regRecord->registerType()));
                        assert(GetNextIntervalRef(reg) == MaxLocation);
                        assert(regRecord->spillCost == 0);
                    }

                    assert(GetNextFixedRef(reg) == regRecord->getNextRefLocation());
        ...
        // boundary first, so that we can free any registers occupied by locals
        ...
        if (currentRefPosition->isPhysRegRef)
        {
            ...
            if (refType == RefTypeFixedReg)
            {
                if (assignedInterval != nullptr && !assignedInterval->isActive && assignedInterval->isConstant)
                {
                    ...
                    regRecord->assignedInterval = nullptr;
                    regRecord->spillCost        = 0;

#ifdef TARGET_ARM
                    if (assignedInterval->registerType == TYP_DOUBLE)
                    {
                        RegRecord* otherReg = regRecord->GetDoublePairOtherReg();
                        assert(otherReg->assignedInterval == assignedInterval);
                        otherReg->assignedInterval = nullptr;
                        otherReg->spillCost        = 0;
                    }
#endif
                }
        ...
        if (refType == RefTypeParamDef || refType == RefTypeZeroInit)
        {
                ...
                blockInfo[compiler->fgFirstBB->bbNum].hasEHPred)
            {
                allocate = false;
                     ...
                     (currentInterval->getLocalVar(compiler)->GetRefWeight() <= BB_UNITY_WEIGHT) &&
            ...
            if (!allocate)
            {
                ...
                if (assignedRegister != REG_NA)
                {
                    ...
                    clearSpillCost(GetRegRecord(assignedRegister), currentInterval->registerType);
        ...
        {
            // This is a save or restore of the upper half of a large vector local.
            Interval* lclInterval = currentInterval->relatedInterval;
            assert(lclInterval->isLocalVar);

            if (refType == RefTypeUpperVectorSave)
            {
                if ((lclInterval->physReg == REG_NA) ||
                    (lclInterval->isPartiallySpilled && (currentInterval->physReg == REG_STK)))
                {
                    allocate = false;
                }
                else
                {
                    lclInterval->isPartiallySpilled = true;
                }
            }
            else if (refType == RefTypeUpperVectorRestore)
            {
                ...
                if (lclInterval->isPartiallySpilled)
                {
                    lclInterval->isPartiallySpilled = false;
                }
                else
                {
                    allocate = false;
        ...
        else if (refType == RefTypeUpperVectorSave)
        {
            ...
            if (assignedRegister != REG_NA)
            {
                unassignPhysReg(GetRegRecord(assignedRegister), currentInterval->firstRefPosition);
        ...
        if (!allocate)
        {
            if (assignedRegister != REG_NA)
            {
                unassignPhysReg(GetRegRecord(assignedRegister), currentRefPosition);
            }
            else if (!didDump)
            {
                INDEBUG(dumpLsraAllocationEvent(LSRA_EVENT_NO_REG_ALLOCATED, currentInterval));
        ...
        if (currentInterval->isSpecialPutArg)
        {
            ...
            if (refType == RefTypeDef)
            {
                ...
                // as fixed reg's next kill location. Otherwise, if source local's next use
                    ...
                    (currentInterval->getNextRefLocation() == GetNextFixedRef(srcInterval->physReg)))
                {
                    ...
                    // Special putarg_reg acts as a pass-thru since both source local
                    // and putarg_reg have the same register allocated.  Physical reg
                    // record of reg continue to point to source locals's interval
                    // instead of to putarg_reg's interval.  So if a spill of reg
                    // allocated to source local happens, to reallocate to another
                    // tree node, before its use at call node it will lead to spill of
                    // local instead of putarg_reg since physical reg record is pointing
                    // to locals's interval. As a result, arg reg would get trashed leading
                    // to bad codegen. The assumption here is that source local of a
        ...
        if (assignedRegister != REG_NA)
        {
            ...
            if (!currentInterval->isActive)
            {
                ...
                else
                {
                    ...
                    updateSpillCost(GetRegRecord(assignedRegister), currentInterval);
                }
        ...
        else if (assignedRegister != REG_NA)
        {
            ...
            if (currentInterval->isLocalVar)
            {
                ...
                if (!keepAssignment)
                {
                    ...
                    unassignPhysRegNoSpill(GetRegRecord(currentInterval->physReg));
        ...
        if (assignedRegister != REG_NA)
        {
            RegRecord* physRegRecord = GetRegRecord(assignedRegister);
            ...
            else if ((genRegMask(assignedRegister) & currentRefPosition->registerAssignment) != 0)
            {
                ...
                if (!currentInterval->isActive)
                {
                    ...
                    {
                        currentInterval->isActive = true;
                        assert(GetRegRecord(assignedRegister)->assignedInterval == currentInterval);
                    }
                    else
                    {
                        currentRefPosition->reload = true;
            ...
            else
            {
                ...
                if (!RefTypeIsDef(currentRefPosition->refType))
                {
                    ...
                    // If we ever actually move local intervals instead of copying, this will need to change.
                    if (!currentInterval->isLocalVar)
                    {
                        currentRefPosition->moveReg = true;
                    ...
                    clearSpillCost(GetRegRecord(copyReg), currentInterval->registerType);
                    updateNextIntervalRef(assignedRegister, currentInterval);
                    updateSpillCost(GetRegRecord(assignedRegister), currentInterval);
                    continue;
                }
...
void LinearScan::updateAssignedInterval(RegRecord* reg, Interval* interval, RegisterType regType)
{
    ...
    RegNum    doubleReg           = REG_NA;

    if (regType == TYP_DOUBLE)
    {
        RegRecord* otherHalfReg        = reg->GetDoublePairOtherReg();
        doubleReg                      = genIsValidDoubleReg(reg->regNum) ? reg->regNum : otherHalfReg->regNum;
        otherHalfReg->assignedInterval = interval;
    }
    else if ((oldAssignedInterval != nullptr) && (oldAssignedInterval->registerType == TYP_DOUBLE))
    {
        RegRecord* otherHalfReg        = reg->GetDoublePairOtherReg();
        doubleReg                      = genIsValidDoubleReg(reg->regNum) ? reg->regNum : otherHalfReg->regNum;
        otherHalfReg->assignedInterval = nullptr;
    }

    if (doubleReg != REG_NA)
    {
        clearNextIntervalRef(doubleReg, TYP_DOUBLE);
        clearSpillCost(GetRegRecord(doubleReg), TYP_DOUBLE);
    ...
    else
    {
        ...
        clearSpillCost(reg, reg->registerType());
    }
}
...
void LinearScan::updatePreviousInterval(RegRecord* reg, Interval* interval, RegisterType regType)
{
    ...
    if (regType == TYP_DOUBLE)
    {
        reg->GetDoublePairOtherReg()->previousInterval = interval;
    }
#endif
}

void LinearScan::writeLocalReg(GenTreeLclVar* lclNode, LclVarDsc* lcl, RegNum reg)
{
    ...
    else
    {
        ...
        // TODO-MIKE-Review: This should assert that local is really a field of the promoted local.
        lclNode->SetRegNum(lcl->GetLclNum() - promotedLcl->GetPromotedFieldLclNum(0), reg);
    }
}

void LinearScan::clearLocalReg(GenTreeLclVar* lclNode, LclVarDsc* lcl)
{
    if (lclNode->GetLcl() == lcl)
    {
        assert(!lclNode->IsMultiReg());
    ...
    else
    {
        ...
        LclVarDsc* promotedLcl = lclNode->GetLcl();
        // TODO-MIKE-Review: This should assert that local is really a field of the promoted local.
        lclNode->ClearRegNum(lcl->GetLclNum() - promotedLcl->GetPromotedFieldLclNum(0));
    }
}
...
// Arguments:
//      node: The local load/store node that's being resolved
...
// fixed register, but it marks the local node with the register it currently lives in
// and the code generator does the necessary move.
//
// Before beginning, the local for each parameter must be set to its initial location.
//
...
void LinearScan::resolveLocalRef(BasicBlock* block, GenTreeLclVar* node, RefPosition* currentRefPosition)
{
    ...
    if ((node != nullptr) && !extendLifetimes())
    {
            ...
            (currentRefPosition->refType == RefTypeUse))
        {
            ...
            assert(inVarToRegMaps[curBBNum][lcl->GetLivenessBitIndex()] == REG_STK);
            currentRefPosition->registerAssignment = RBM_NONE;
    ...
    if (!currentRefPosition->copyReg)
    {
        ...
        {
            RegRecord* oldRegRecord = GetRegRecord(oldAssignedReg);
        ...
        if (node != nullptr)
        {
            ...
            if (spillAfter)
            {
                if (currentRefPosition->RegOptional())
                {
                    ...
                    // by setting reg on local to REG_STK and reg on tree node
    ...
    else // Not reload and Not pure-def that's spillAfter
    {
        if (currentRefPosition->copyReg || currentRefPosition->moveReg)
        {
            ...
            assert(node != nullptr);
            writeLocalReg(node, lcl, interval->physReg);
    ...
    RegRecord* physRegRecord = GetRegRecord(homeReg);
...
void LinearScan::insertCopyOrReload(BasicBlock* block, GenTree* value, unsigned regIndex, RefPosition* refPosition)
{
    ...
    if ((regType == TYP_STRUCT) && !value->IsMultiRegNode())
    {
        ...
        const GenTreeLclRef* load = value->AsLclRef();
...
#if FEATURE_PARTIAL_SIMD_CALLEE_SAVE
// Insert code to save the upper half of a vector that lives in a callee-save
// register at the point of a kill (the upper half is not preserved).
void LinearScan::InsertUpperVectorSpill(GenTree*     before,
                                        RefPosition* refPosition,
                                        Interval*    upperVectorInterval,
                                        BasicBlock*  block)
{
    JITDUMP("Inserting UpperVectorSpill for RP #%d before %u.%s:\n", refPosition->rpNum, before->GetID(),
            GenTree::OpName(before->GetOper()));

    Interval* lclInterval = upperVectorInterval->relatedInterval;
    assert(lclInterval->isLocalVar);
    assert(refPosition->getInterval() == upperVectorInterval);

    RegNum lclReg = lclInterval->physReg;
    ...
    LclVarDsc* lcl = lclInterval->getLocalVar(compiler);
    assert(varTypeNeedsPartialCalleeSave(lcl->GetRegisterType()));

    // On Arm64, we must always have a register to save the upper half,
    // while on x86 we can spill directly to memory.
    RegNum spillReg = refPosition->assignedReg();
#ifdef TARGET_ARM64
    ...
    GenTree* lclUse = compiler->gtNewLclLoad(lcl, lcl->GetType());
    lclUse->SetRegNum(lclReg);
    lclUse->ClearRegSpillSet();
    SetLsraAdded(lclUse);

    GenTree* spillNode = compiler->gtNewOperNode(GT_SIMD_UPPER_SPILL, LargeVectorSaveType, lclUse);
    spillNode->ClearRegSpillSet();
    SetLsraAdded(spillNode);

#ifdef TARGET_ARM64
    spillNode->SetRegNum(spillReg);
#endif

    if (spillToMem)
    {
        spillNode->SetRegSpill(0, true);
...
// Insert code to restore the upper half of a vector that has been partially spilled.
// In the case where 'before' is non-null, we will insert the restore just prior to
// its use, in order to ensure the proper ordering.
void LinearScan::InsertUpperVectorUnspill(GenTree*     before,
                                          RefPosition* refPosition,
                                          Interval*    upperVectorInterval,
                                          BasicBlock*  block)
{
    JITDUMP("Adding UpperVectorUnspill for RP #%d ", refPosition->rpNum);

    Interval* lclInterval = upperVectorInterval->relatedInterval;
    assert(lclInterval->isLocalVar);

    RegNum lclReg = lclInterval->physReg;
    // We should not call this method if the local is not in a register
    // (we should have  simply marked the entire local as spilled).
    assert(lclReg != REG_NA);
    ...
    GenTree* unspillNode = compiler->gtNewOperNode(GT_SIMD_UPPER_UNSPILL, lcl->GetType(), lclUse);
    unspillNode->ClearRegSpillSet();
    SetLsraAdded(unspillNode);

    RegNum restoreReg = upperVectorInterval->physReg;

    if (restoreReg == REG_NA)
    {
        // We need a stack location for this.
        assert(lclInterval->isSpilled);
#ifdef TARGET_AMD64
        assert(refPosition->assignedReg() == REG_NA);
        unspillNode->gtFlags |= GTF_NOREG_AT_USE;
#else
        unspillNode->SetRegSpilled(0, true);
        assert(refPosition->assignedReg() != REG_NA);
        restoreReg = refPosition->assignedReg();
        unspillNode->SetRegNum(restoreReg);
#endif
    }
    else
    {
    ...
    if (before != nullptr)
    {
        JITDUMP("before %u.%s:\n", before->GetID(), GenTree::OpName(before->GetOper()));

        LIR::Use beforeUse;
        bool     foundUse = blockRange.TryGetUse(before, &beforeUse);
        assert(foundUse);
        // We need to insert the restore prior to the use, not (necessarily) immediately after the local.
        blockRange.InsertBefore(beforeUse.User(), lclUse, unspillNode);
    }
    else
    {
        ...
        if (block->KindIs(BBJ_COND, BBJ_SWITCH))
        {
            ...
            blockRange.InsertBefore(branch, lclUse, unspillNode);
        }
        else
        {
            assert(block->bbJumpKind == BBJ_NONE || block->bbJumpKind == BBJ_ALWAYS);
            blockRange.InsertAfter(blockRange.LastNode(), lclUse, unspillNode);
        }
    }
...
void LinearScan::updateMaxSpill(RefPosition* refPosition)
{
    ...
    if ((refType == RefTypeUpperVectorSave) || (refType == RefTypeUpperVectorRestore))
    {
        ...
        else
        {
            ...
            Interval* lclInterval = interval->relatedInterval;
            assert(lclInterval->isSpilled || (!refPosition->spillAfter && !refPosition->reload));
        }
        return;
    }
#endif // !FEATURE_PARTIAL_SIMD_CALLEE_SAVE
...
void LinearScan::resolveRegisters()
{
    ...
    // Clear register assignments - these will be reestablished as local defs (including RefTypeParamDefs)
    // are encountered.
    if (enregisterLocalVars)
    {
    ...
    for (unsigned blockSeqIndex = 0, blockSeqCount = bbSeqCount; blockSeqIndex < blockSeqCount; blockSeqIndex++)
    {
             ...
             ++refPosIterator)
        {
            ...
            if (refPosIterator->refType == RefTypeUpperVectorSave)
            {
                ...
                // LocalVar.
                // Otherwise, this  is a non-local interval that has been spilled, and we don't need to do anything.
                Interval* interval = refPosIterator->getInterval();
                ...
                if (interval->isUpperVector)
                {
                    ...
                    if ((localVarInterval->physReg != REG_NA) && !localVarInterval->isPartiallySpilled)
                    {
                        ...
                        refPosIterator->referent->recentRefPosition = &refPosIterator;
                        InsertUpperVectorSpill(node, &refPosIterator, refPosIterator->getInterval(), block);
                        localVarInterval->isPartiallySpilled = true;
                    }
                }
                else
                {
                    // This is a non-local interval that must have been spilled.
            ...
            if (refPosIterator->refType == RefTypeUpperVectorRestore)
            {
                ...
                if (localVarInterval->physReg != REG_NA)
                {
                    ...
                    InsertUpperVectorUnspill(node, &refPosIterator, interval, block);
                }
            ...
            if (node == nullptr)
            {
                ...
                if (interval->isLocalVar && !interval->isStructField)
                {
                    ...
                    interval->getLocalVar(compiler)->SetRegNum(REG_STK);
                }
        ...
        for (LclVarDsc* lcl : compiler->LivenessLocals())
        {
            ...
            if ((lcl->GetRegNum() == REG_STK) || interval->isSpilled || interval->isSplit)
            {
                ...
                else
                {
                    ...
                    {
                        lcl->SetRegNum(firstRefPosition->assignedReg());
                    }
                }
            }
            else
...
void LinearScan::handleOutgoingCriticalEdges(BasicBlock* block, VARSET_TP outResolutionSet)
{
    ...
    for (VarSetOps::Enumerator e(compiler, outResolutionSet); e.MoveNext();)
    {
        ...
        if (sameToReg != REG_NA && sameToReg != REG_STK)
        {
            ...
            // Similarly, if sameToReg is in sameWriteRegs, it has already been used (i.e. for a local that's
            // live only at another target), and we can't copy another local into that reg in this block.
...
void LinearScan::resolveEdges()
{
    ...
    // The resolutionCandidateVars set was initialized with all the locals that are live-in to
    // any block. We now intersect that set with any locals that ever spilled or split.
...
//    liveSet       - the set of tracked local indices which may require resolution
//
// Return Value:
//    None.
//
...
void LinearScan::resolveEdge(BasicBlock* fromBlock, BasicBlock* toBlock, ResolveType resolveType, VARSET_TP liveSet)
{
...
#endif
    RegNum tempRegFlt = REG_NA;
#ifdef TARGET_ARM
    RegNum tempRegDbl = REG_NA;
#endif
    ...
    for (VarSetOps::Enumerator e(compiler, liveSet); e.MoveNext();)
    {
        ...
        Interval* const interval = getIntervalForLocalVar(varIndex);
        RegNum const    fromReg  = getVarReg(fromVarToRegMap, varIndex);
        RegNum const    toReg    = getVarReg(toVarToRegMap, varIndex);
        ...
        assert((REG_FIRST <= toReg) && (toReg <= REG_LAST));
        ...
        assert((REG_FIRST <= fromReg) && (fromReg <= REG_LAST));

        location[fromReg]  = static_cast<RegNumSmall>(fromReg);
        source[toReg]      = static_cast<RegNumSmall>(fromReg);
        intervals[fromReg] = interval;
    ...
    {
        regMaskTP targetRegBit = genFindLowestBit(targetCandidates);
        targetCandidates &= ~targetRegBit;
        RegNum targetReg = genRegNumFromMask(targetRegBit);
        ...
        if (intervals[sourceReg]->registerType == TYP_DOUBLE)
        {
            assert(genIsValidDoubleReg(targetReg));

            if (location[REG_NEXT(targetReg)] != REG_NA)
            {
                continue;
            }
        }
    ...
    while (targetRegsToDo != RBM_NONE)
    {
        ...
        {
            regMaskTP targetRegBit = genFindLowestBit(targetRegsReady);
            targetRegsToDo &= ~targetRegBit;
            targetRegsReady &= ~targetRegBit;
            RegNum targetReg = genRegNumFromMask(targetRegBit);
            ...
            assert(fromReg < REG_COUNT);
            Interval* interval = intervals[sourceReg];
            assert(interval != nullptr);

            InsertRegCopy(block, insertionPoint, interval, targetReg, fromReg);
            JITDUMP(" (%s)\n", resolveTypeName[resolveType]);

            intervals[sourceReg] = nullptr;
            ...
            if ((source[fromReg] != REG_NA) && ((targetRegsFromStack & fromRegMask) != fromRegMask))
            {
                ...
                if (genIsValidDoubleReg(fromReg))
                {
                    ...
                    if ((intervals[source[fromReg]]->registerType == TYP_DOUBLE) &&
                        (location[REG_NEXT(fromReg)] != REG_NA))
                    {
                        targetRegsReady &= ~fromRegMask;
                    }
                }
            ...
            else if (genIsValidFloatReg(fromReg) && !genIsValidDoubleReg(fromReg))
            {
                ...
                // was already free.
                RegNum    lowerHalfReg     = REG_PREV(fromReg);
                RegNum    lowerHalfSrcReg  = static_cast<RegNum>(source[lowerHalfReg]);
                RegNum    lowerHalfSrcLoc  = static_cast<RegNum>(location[lowerHalfReg]);
                ...
                if ((lowerHalfSrcReg != REG_NA) && (lowerHalfSrcLoc == REG_NA) &&
                    (intervals[lowerHalfSrcReg] != nullptr) && ((targetRegsReady & lowerHalfRegMask) == RBM_NONE) &&
                    ((targetRegsFromStack & lowerHalfRegMask) != lowerHalfRegMask))
                {
                    // This must be a double interval, otherwise it would be in targetRegsReady,
                    // or already completed.
                    assert(intervals[lowerHalfSrcReg]->registerType == TYP_DOUBLE);
        ...
        RegNum    targetReg     = genRegNumFromMask(targetRegMask);
        RegNum    sourceReg     = static_cast<RegNum>(source[targetReg]);
        RegNum    fromReg       = static_cast<RegNum>(location[sourceReg]);
        ...
        RegNum tempReg = REG_NA;
        bool   useSwap = false;

        if (IsFloatReg(targetReg))
        {
            tempReg = ARM_ONLY((intervals[fromReg]->registerType == TYP_DOUBLE) ? tempRegDbl :) tempRegFlt;
        }
        else
        {
#ifdef TARGET_XARCH
        ...
        if (useSwap || (tempReg == REG_NA))
        {
            ...
            if (location[source[fromReg]] == targetReg)
            {
                ...
                if (useSwap)
                {
                    targetRegsToDo &= ~genRegMask(fromReg);
                }
            }
            else
            {
                ...
                for (regMaskTP mask = targetRegsToDo; (mask != RBM_NONE) && (otherTargetReg == REG_NA);)
                {
                    regMaskTP nextRegMask = genFindLowestBit(mask);
                    mask &= ~nextRegMask;
                    RegNum nextReg = genRegNumFromMask(nextRegMask);
            ...
            if (useSwap)
            {
                InsertRegSwap(block, insertionPoint, intervals[source[otherTargetReg]], targetReg, intervals[sourceReg],
                              fromReg);
                location[sourceReg]              = REG_NA;
                location[source[otherTargetReg]] = static_cast<RegNumSmall>(fromReg);
...
#endif // TARGET_XARCH
            {
                ...
                Interval* otherInterval = intervals[source[otherTargetReg]];
                ...
                InsertRegCopy(block, insertionPoint, intervals[sourceReg], targetReg, fromReg);
                JITDUMP(" (%s)\n", resolveTypeName[resolveType]);
                location[sourceReg] = REG_NA;
...
#ifdef TARGET_ARM
                    if (genIsValidDoubleReg(fromReg) && (intervals[source[fromReg]]->registerType == TYP_DOUBLE) &&
                        location[REG_NEXT(fromReg)] != REG_NA)
                    {
                        targetRegsReady &= ~fromRegMask;
                    }
        ...
        // TODO-MIKE-Review: Does this need handling for DOUBLE on ARM?
        m_allocateRegs |= genRegMask(tempReg);

#ifdef TARGET_ARM
        if (intervals[fromReg]->registerType == TYP_DOUBLE)
        {
            InsertDoubleRegCopy(block, insertionPoint, intervals, location, tempReg, targetReg DEBUG_ARG(resolveType));
        }
        else
#endif
        {
            assert(intervals[targetReg] != nullptr);

            InsertRegCopy(block, insertionPoint, intervals[targetReg], tempReg, targetReg);
            JITDUMP(" (%s)\n", resolveTypeName[resolveType]);
            location[targetReg] = static_cast<RegNumSmall>(tempReg);
        }
    ...
    {
        regMaskTP targetRegBit = genFindLowestBit(targetRegsFromStack);
        targetRegsFromStack &= ~targetRegBit;
        RegNum targetReg = genRegNumFromMask(targetRegBit);

        InsertRegLoad(block, insertionPoint, stackToRegIntervals[targetReg], targetReg);
        JITDUMP(" (%s)\n", resolveTypeName[resolveType]);
    }
}
...
void LinearScan::lsraDispNode(GenTree* node, LsraTupleDumpMode mode) const
{
    ...
    if (mode == LinearScan::LSRA_DUMP_POST)
    {
        ...
        if (!hasDest && node->HasRegs())
        {
            ...
            // However, we can have unused locals that wind up being the node at which
            ...
            else
            {
                     ...
                     ++refPosIterator)
                {
                    ...
                    case RefTypeUse:
                        ...
                        {
                            printf("\n                               Use:R%d(#%d)", refPosIterator->getReg()->regNum,
                                   refPosIterator->rpNum);
                        }
                        else
                        {
...
void LinearScan::dumpRegRecords()
{
    ...
    for (regNumber regNum = REG_FIRST; regNum <= (regNumber)lastUsedRegNumIndex; regNum = REG_NEXT(regNum))
    {
        if (shouldDumpReg(regNum))
        {
            ...
            if (Interval* interval = GetRegRecord(regNum)->assignedInterval)
            {
...
void LinearScan::dumpRefPositionShort(RefPosition* refPosition, BasicBlock* currentBlock)
{
    ...
    {
        printf(regNameFormat, getRegName(refPosition->getReg()->regNum));
...
// Traverse the RefPositions and verify various invariants.
...
void LinearScan::verifyFinalAllocation()
{
    ...
    for (RefPosition& refPosition : refPositions)
    {
            ...
            if (currentBlock != nullptr)
            {
                if (enregisterLocalVars)
                {
                    ...
                    for (VarSetOps::Enumerator e(compiler, currentBlock->bbLiveIn); e.MoveNext();)
                    {
                        ...
                        RegNum regNum = getVarReg(inVarToRegMap, varIndex);
                        ...
                        if (regNum <= REG_LAST)
                        {
                            ...
                            RegRecord* regRecord        = GetRegRecord(regNum);
        ...
        case RefTypeZeroInit:
            ...
            else if (RefTypeIsDef(currentRefPosition->refType))
            {
                ...
                DBEXEC(verbose, dumpLsraAllocationEvent(interval->isConstant && (currentRefPosition->node != nullptr) &&
                                                                currentRefPosition->node->IsReuseRegVal()
            ...
            if (regNum != REG_NA)
            {
                if (currentRefPosition->spillAfter)
                {
                    if (verbose)
                    {
                        ...
                        dumpRegRecords();
                        dumpEmptyRefPosition();
                        printf("%s    %-4s ", currentRefPosition->writeThru ? "WThru" : "Spill", getRegName(spillReg));
                    }
                }
                else if (currentRefPosition->copyReg)
                {
                ...
                else
                {
                    if (RefTypeIsDef(currentRefPosition->refType))
                    {
                        ...
                        if (interval->assignedReg != nullptr && interval->physReg != regNum)
                        {
                            interval->assignedReg->assignedInterval = nullptr;
                        }
                    }
        ...
        case RefTypeKillGCRefs:
            ...
            for (regMaskTP candidateRegs = currentRefPosition->registerAssignment; candidateRegs != RBM_NONE;)
            {
                regMaskTP nextRegBit = genFindLowestBit(candidateRegs);
                candidateRegs &= ~nextRegBit;
                Interval* assignedInterval = GetRegRecord(genRegNumFromMask(nextRegBit))->assignedInterval;
        ...
        if (currentRefPosition->refType != RefTypeBB)
        {
            ...
            if (interval != nullptr)
            {
                ...
                if (currentRefPosition->spillAfter || currentRefPosition->lastUse)
                {
                    ...
                    {
                        // These only require a register if they are not RegOptional, and their local
                        ...
                        {
                            Interval* lclInterval = interval->relatedInterval;
                            assert((lclInterval->physReg == REG_NA) || lclInterval->isPartiallySpilled);
                        }
                    }
#endif
                    else
    ...
    for (BasicBlock* const currentBlock : compiler->Blocks())
    {
        ...
        {
            // If we haven't enregistered any locals, we have no resolution blocks.
            assert(enregisterLocalVars);
...
void LinearScan::verifyResolutionMove(GenTree* resolutionMove, LsraLocation currentLocation)
{
    ...
    if (dst->OperIs(GT_SWAP))
    {
        ...
        RegNum          leftRegNum     = left->GetRegNum();
        RegNum          rightRegNum    = right->GetRegNum();
        LclVarDsc*      leftLcl        = left->GetLcl();
        LclVarDsc*      rightLcl       = right->GetLcl();
        Interval*       leftInterval   = getIntervalForLocalVar(leftLcl->GetLivenessBitIndex());
        Interval*       rightInterval  = getIntervalForLocalVar(rightLcl->GetLivenessBitIndex());
        RegRecord*      rightRegRecord = GetRegRecord(rightRegNum);
        RegRecord*      leftRegRecord  = GetRegRecord(leftRegNum);
        ...
        leftInterval->assignedReg        = rightRegRecord;
        rightInterval->assignedReg       = leftRegRecord;
        rightRegRecord->assignedInterval = leftInterval;
        leftRegRecord->assignedInterval  = rightInterval;
    ...
    {
        GetRegRecord(srcRegNum)->assignedInterval = nullptr;
    }

    if (dstRegNum != REG_STK)
    {
        interval->physReg                         = dstRegNum;
        interval->assignedReg                     = GetRegRecord(dstRegNum);
        GetRegRecord(dstRegNum)->assignedInterval = interval;
        interval->isActive                        = true;
    }
    else
    {
...
void RegisterSelection::reset(Interval* interval, RefPosition* refPos)
{
    ...
    // This is not actually a preference, it's merely to track the local that this "specialPutArg" is using.
...
bool RegisterSelection::try_REG_ORDER()
{
    ...
    for (regMaskTP regOrderCandidates = candidates; regOrderCandidates != RBM_NONE;)
    {
        ...
        unsigned thisRegOrder = linearScan->GetRegRecord(genRegNumFromMask(regOrderCandidateBit))->regOrder;
...
bool RegisterSelection::try_SPILL_COST()
{
    ...
    for (regMaskTP spillCandidates = candidates; spillCandidates != RBM_NONE;)
    {
        ...
        RegNum     spillCandidateRegNum    = genRegNumFromMask(spillCandidateBit);
        RegRecord* spillCandidateRegRecord = linearScan->GetRegRecord(spillCandidateRegNum);
        ...
        {
            currentSpillWeight = spillCandidateRegRecord->spillCost;

#ifdef TARGET_ARM
            if (currentInterval->registerType == TYP_DOUBLE)
            {
                currentSpillWeight =
                    Max(currentSpillWeight, spillCandidateRegRecord->GetDoublePairNextReg()->spillCost);
            }
#endif
        }
...
bool RegisterSelection::try_PREV_REG_OPT()
{
    ...
    for (regMaskTP prevRegOptCandidates = candidates; prevRegOptCandidates != RBM_NONE;)
    {
        ...
        RegNum     prevRegOptCandidateRegNum = genRegNumFromMask(prevRegOptCandidateBit);
        RegRecord* prevRegOptCandidateReg    = linearScan->GetRegRecord(prevRegOptCandidateRegNum);

        Interval* assignedInterval   = prevRegOptCandidateReg->assignedInterval;
        bool      foundPrevRegOptReg = true;
        INDEBUG(bool hasAssignedInterval = false);
        ...
        if ((assignedInterval != nullptr) && (assignedInterval->recentRefPosition != nullptr))
        {
            ...
            INDEBUG(hasAssignedInterval = true);
        }
#ifndef TARGET_ARM
        else
        {
        ...
        if (currentInterval->registerType == TYP_DOUBLE)
        {
            assignedInterval = prevRegOptCandidateReg->GetDoublePairOtherReg()->assignedInterval;
...
void RegisterSelection::calculateCoversSets()
{
    ...
    while (coversCandidates != RBM_NONE)
    {
        ...
        if (linearScan->GetNextIntervalRef(coversCandidateRegNum) > lastLocation)
        {
...
void RegisterSelection::resolveConflictingDefAndUse()
{
    ...
    if (defRefPosition->isFixedRegRef && !defRegConflict)
    {
        ...
        defRegRecord = linearScan->GetRegRecord(defReg);
    ...
    if (useRefPosition->isFixedRegRef && !useRegConflict)
    {
        ...
        useRegRecord = linearScan->GetRegRecord(useReg);
...
regMaskTP LinearScan::getMatchingConstants(regMaskTP mask, Interval* interval, RefPosition* refPosition)
{
    ...
    while (candidates != RBM_NONE)
    {
        ...
        RegRecord* regRecord = GetRegRecord(genRegNumFromMask(candidateBit));
...
regMaskTP RegisterSelection::select(Interval* currentInterval, RefPosition* refPosition)
{
    ...
    if (RefTypeIsDef(refPosition->refType))
    {
                 ...
                 !nextRefPos->isFixedRegRef && genMaxOneBit(refPosition->registerAssignment))
        {
            ...
            RegRecord* defRegRecord = linearScan->GetRegRecord(defReg);
                ...
                RBM_NONE X86_ONLY(&&(!avoidByteRegs || thisRelatedPreferences != RBM_BYTE_REGS)))
            {
                ...
                if (!thisIsSingleReg ||
                    (finalRelatedInterval->isLocalVar &&
                     linearScan->isFree(linearScan->GetRegRecord(genRegNumFromMask(newRelatedPreferences)))))
                {
    ...
    if (refPosition->isFixedRegRef)
    {
        ...
        if (candidates == refPosition->registerAssignment)
        {
            ...
            if (linearScan->GetNextIntervalRef(genRegNumFromMask(candidates)) > lastLocation)
            {
    ...
    if (!found)
    {
        ...
        while (checkConflictMask != RBM_NONE)
        {
            ...
            RegNum       checkConflictReg      = genRegNumFromMask(checkConflictBit);
