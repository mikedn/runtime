...
//    upperInterval     - The Interval for the upper half of the large vector local.
                                       ...
                                       BasicBlock*  block)
{
    ...
    regNumber lclReg = lclInterval->physReg;
    if (lclReg == REG_NA)
    {
        return;
    }
...
//                          If non-null, it will be a use of the large vector local.
//                          If null, the Restore will be added to the end of the block.
//    upperVectorInterval - The Interval for the upper vector for the local.
...
void LinearScan::resolveEdge(BasicBlock* fromBlock, BasicBlock* toBlock, ResolveType resolveType, VARSET_TP liveSet)
{
    ...
    for (VarSetOps::Enumerator e(compiler, liveSet); e.MoveNext();)
    {
        ...
        assert((REG_FIRST <= toReg) && (toReg <= REG_MASK_LAST));
        ...
        assert((REG_FIRST <= fromReg) && (fromReg <= REG_MASK_LAST));
    ...
    while (targetRegsToDo != RBM_NONE)
    {
        while (targetRegsReady != RBM_NONE)
        {
            ...
            assert(targetReg < REG_MASK_COUNT);
            RegNum sourceReg = static_cast<RegNum>(source[targetReg]);
            assert(sourceReg < REG_MASK_COUNT);
            RegNum fromReg = static_cast<RegNum>(location[sourceReg]);
            assert(fromReg < REG_MASK_COUNT);
