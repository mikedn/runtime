    ...
    void GenerateNodeAssertions(GenTree* node)
    {
        ...
        switch (node->GetOper())
        {
            case GT_BOUNDS_CHECK:
                // TODO-MIKE-Review: See if assertions generated by vector indexing are of any use,
                // probably not. But then they're probably rare so generating some useless assertion
                // wouldn't matter too much, then this is just a pointless check.
                if ((node->gtFlags & GTF_BOUND_VECT) == 0)
                {
