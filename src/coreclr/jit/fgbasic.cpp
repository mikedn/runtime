    ...
    DECODE_OPCODE:
        ...
        case CEE_CLT_UN:
            ...
            else
            {
                ...
                else if (FgStack::IsConstArgument(arg0, inlineInfo) && FgStack::IsConstantOrConstArg(arg1, inlineInfo))
                {
                ...
                {
                    if ((opcode == CEE_DIV) || (opcode == CEE_DIV_UN) || (opcode == CEE_REM) || (opcode == CEE_REM_UN))
                    {
        ...
        case CEE_BNE_UN_S:
        {
            ...
            {
                BADCODE3("code jumps to outer space", " at offset %04X", static_cast<unsigned>(codeAddr - codeBegin));
            }
            ...
            case CEE_BLT_UN_S:
            {
                ...
                if (FgStack::IsConstantOrConstArg(op1, inlineInfo) && FgStack::IsConstantOrConstArg(op2, inlineInfo))
                {
...
unsigned Compiler::fgMakeBasicBlocks(ILLabelSet jumpTargets)
{
    ...
    if (opts.compDbgCode && (info.compVarScopesCount > 0) && !info.compVarScopeExtended)
    {
        ...
        while (compGetNextEnterScope(0, &nextEnterScope))
        {
        }

        while (compGetNextExitScope(0, &nextEnterScope))
        {
        }
    }
    ...
    DECODE_OPCODE:
        ...
        if (opts.compDbgCode && (info.compVarScopesCount > 0) && !info.compVarScopeExtended)
        {
            ...
            while (compGetNextExitScope(nextBlockOffset, &nextExitScope))
            {
                foundScope = true;
            }
        }
...
BasicBlock* Compiler::fgSplitBlockAfterNode(BasicBlock* curr, GenTree* node)
{
    ...
    if (node != nullptr)
    {
        ...
        for (riter = currBBRange.rbegin(), riterEnd = currBBRange.rend(); riter != riterEnd; ++riter)
        {
            ...
            {
                if (ilOffset->HasILOffset())
                {
                    splitPointILOffset = ilOffset->GetILOffset();
                    break;
                }
            }
        }
