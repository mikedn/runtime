    ...
    switch (intrinsic)
    {
        case NI_AVX2_PermuteVar8x32:
        {
            ...
            if (!gtCanSwapOrder(control, left))
            {
                ...
                LclVarDsc* tempLcl = lvaGrabTemp(true DEBUGARG("AVX2.PermuteVar8x32 temp"));
                impAppendTempAssign(tempLcl, left, sig.paramLayout[0], CHECK_SPILL_ALL);
                left = gtNewLclvNode(tempLcl->GetLclNum(), sig.paramType[0]);
            }
    ...
    switch (intrinsic)
    {
        ...
        case NI_BMI2_X64_ZeroHighBits:
        {
            ...
            if (!gtCanSwapOrder(op1, op2))
            {
                ...
                LclVarDsc* tempLcl = lvaGrabTemp(true DEBUGARG("BMI.BitFieldExtract/ZeroHightBits temp"));
                impAppendTempAssign(tempLcl, op1, CHECK_SPILL_ALL);
                op1 = gtNewLclvNode(tempLcl->GetLclNum(), varActualType(sig.paramType[0]));
            }
