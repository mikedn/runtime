...
class Cse
{
    ...
    struct Value
    {
        ...
        ValueNum useExset = ValueNumStore::VNForEmptyExcSet();
                ...
                for (GenTree* const expr : stmt->Nodes())
                {
                    ...
                    if (isDef)
                    {
                        if (value->defExset != exprExset)
                        {
                            ...
                            {
                                value->defExset = vnStore->VNExcSetIntersection(value->defExset, exprExset);
                            }
                    ...
                    else
                    {
                        if (exprExset != ValueNumStore::VNForEmptyExcSet())
                        {
                            ...
                            {
                                value->useExset = vnStore->VNExcSetUnion(value->useExset, exprExset);
                            }
                        }
        ...
        for (const Occurrence* occ = &value->firstOccurrence; occ != nullptr; occ = occ->next)
        {
            ...
            if (IsCseDef(expr->GetCseInfo()))
            {
                ...
                if (!value->isSharedConst)
                {
                    ...
                    if (conservativeVN == ValueNumStore::VNForVoid())
                    {
                        conservativeVN = vn;
                    }
