...
bool GenTreeCall::Equals(GenTreeCall* c1, GenTreeCall* c2)
{
    ...
    GenTreeCall::UseIterator i1   = c1->Uses().begin();
    GenTreeCall::UseIterator end1 = c1->Uses().end();
    GenTreeCall::UseIterator i2   = c2->Uses().begin();
    GenTreeCall::UseIterator end2 = c2->Uses().end();
...
AGAIN:
    ...
    if (kind & GTK_BINOP)
    {
        if (GenTree::IsExOp(kind))
        {
            ...
            case GT_INTRINSIC:
                ...
                break;
            case GT_LEA:
                hash += static_cast<unsigned>(tree->AsAddrMode()->GetOffset() << 3) + tree->AsAddrMode()->GetScale();
                break;
    ...
    case GT_CALL:
        for (GenTreeUse& use : tree->AsCall()->Uses())
        {
            hash = genTreeHashAdd(hash, gtHashValue(use.GetNode()));
        }
...
unsigned Compiler::gtSetOrder(GenTree* tree)
{
    ...
    case GT_CALL:
    {
        ...
        if (call->m_uses != nullptr)
        {
            level = Max(level, gtSetCallArgsOrder(call->Uses()));
        }
...
GenTreeCall::Use* Compiler::gtPrependNewCallArg(GenTreeCall* call, GenTree* node)
{
    ...
    return gtPrependNewCallArg(call->m_uses, node);
}
...
{
    assert(!call->HasArgsSetup());
    gtAppendCallArgs(call->m_uses, args);
}
...
{
    assert(!call->HasArgsSetup());
    return gtAppendNewCallArg(call->m_uses, node);
}
...
GenTree* Compiler::gtCloneExpr(GenTree* tree, GenTreeFlags addFlags, const LclVarDsc* constLcl, const int constVal)
{
    ...
    if ((kind & (GTK_UNOP | GTK_BINOP)) != 0)
    {
        ...
        case GT_FREM:
            ...
            copy =
                new (this, GT_CALL) GenTreeOp(GT_FREM, tree->GetType(), tree->AsOp()->GetOp(0), tree->AsOp()->GetOp(1));
    ...
    case GT_HWINTRINSIC:
        ...
        {
            copy->AsHWIntrinsic()->SetOp(i, gtCloneExpr(tree->AsHWIntrinsic()->GetOp(i), addFlags, constLcl, constVal));
        }
        break;
#endif
                                             ...
                                             const int        constVal)
{
    ...
    GenTreeCall::Use** argsTail = &copy->m_uses;
    for (GenTreeCall::Use& use : tree->Uses())
    {
...
GenTreeUseEdgeIterator::GenTreeUseEdgeIterator(GenTree* node) : m_node(node)
{
    ...
    case GT_HWINTRINSIC:
        ...
        {
            m_state   = m_node->AsHWIntrinsic()->Uses().begin();
    ...
    default:
        ...
        {
            m_edge    = &m_node->AsOp()->gtOp1;
            m_advance = &GenTreeUseEdgeIterator::AdvanceBinOp1;
        }
        return;
    }
}
...
{
    if (m_state == m_node->AsArrElem()->Uses().end())
    {
        return nullptr;
    }
...
{
    if (m_state == m_node->AsInstr()->Uses().end())
    {
        return nullptr;
    }
...
{
    if (m_state == m_node->AsHWIntrinsic()->Uses().end())
    {
        return nullptr;
    }
    ...
    GenTree* tree, IndentStack* indentStack, const char* msg, bool topOnly, bool isLIR, bool header)
{
    ...
    case GT_HWINTRINSIC:
        printf(" %s %s %u", GetHWIntrinsicIdName(tree->AsHWIntrinsic()->GetIntrinsic()),
               tree->AsHWIntrinsic()->GetVecEltType() == TYP_UNDEF ? "" : varTypeName(
                                                                              tree->AsHWIntrinsic()->GetVecEltType()),
    ...
    case GT_INSTR:
        ...
        if (!topOnly)
        {
            ...
            {
                gtDispChild(tree->AsInstr()->GetOp(i), (i == tree->AsInstr()->GetNumOps() - 1) ? IIArcBottom : IIArc);
            }
        }
        break;
...
void Compiler::gtGetCallArgMsg(GenTreeCall* call, GenTree* arg, unsigned argNum, char* buf, unsigned bufLength)
{
    ...
    {
        if (call->HasThisArg() && (arg == call->GetThisArg()->GetNode()))
        {
...
GenTree* Compiler::gtFoldTypeCompare(GenTree* tree)
{
    ...
    if ((op1Kind == TPK_GetType) && (op2Kind == TPK_GetType))
    {
        ...
        {
            arg1 = op1->AsCall()->GetThisArg()->GetNode();
        }
        ...
        {
            arg2 = op2->AsCall()->GetThisArg()->GetNode();
        }
...
GenTree* Compiler::gtTryRemoveBoxUpstreamEffects(GenTreeBox* box, BoxRemovalOptions options)
{
    ...
    if ((options == BR_REMOVE_AND_NARROW_WANT_TYPE_HANDLE) || (options == BR_DONT_REMOVE_WANT_TYPE_HANDLE))
    {
        ...
        {
            GenTreeCall::Use* args = allocCall->m_uses;
...
CORINFO_CLASS_HANDLE Compiler::gtGetClassHandle(GenTree* tree, bool* isExact, bool* isNonNull)
{
    ...
    *isExact   = false;
    ...
    case GT_LCL_LOAD:
    {
        ...
        *isExact       = lcl->lvClassIsExact;
        return lcl->lvClassHnd;
    }

    case GT_RET_EXPR:
        return gtGetClassHandle(tree->AsRetExpr()->GetRetExpr(), isExact, isNonNull);
    ...
    case GT_CALL:
    {
        ...
        if (call->IsSpecialIntrinsic())
        {
            ...
            {
                return gtGetClassHandle(call->GetThisArg()->GetNode(), isExact, isNonNull);
            }

            if (CORINFO_CLASS_HANDLE retClass = gtGetSpecialIntrinsicExactReturnType(call))
            {
                *isExact   = true;
                *isNonNull = true;
                return retClass;
        ...
        if (InlineCandidateInfo* inlineInfo = call->HasInlinedCandidateInfo())
        {
            ...
            assert(inlineInfo->methInfo.args.retType == CORINFO_TYPE_CLASS);
            CORINFO_CLASS_HANDLE retClass = inlineInfo->methInfo.args.retTypeClass;
            ...
            if ((info.compCompHnd->getClassAttribs(retClass) & CORINFO_FLG_SHAREDINST) != 0)
            {
        ...
        {
            return gtGetHelperCallClassHandle(call, isExact, isNonNull);
        }
    ...
    case GT_INTRINSIC:
        if (obj->AsIntrinsic()->GetIntrinsic() == NI_CORINFO_INTRINSIC_Object_GetType)
        {
            *isExact   = false;
            *isNonNull = true;
            return info.compCompHnd->getBuiltinClass(CLASSID_RUNTIME_TYPE);
        }
        break;
    ...
    case GT_IND_LOAD:
    {
        ...
        if (GenTreeLclAddr* lclAddr = addr->IsLocalAddrExpr())
        {
            ...
            *isExact = objLcl->lvClassIsExact;
            return objLcl->lvClassHnd;
        }
        else if (GenTreeFieldAddr* field = addr->IsFieldAddr())
        {
            ...
            if (fieldSeq->IsField())
            {
                return gtGetFieldClassHandle(fieldSeq->GetFieldHandle(), isExact, isNonNull);
            }
        }
        else if (GenTreeIndexAddr* index = addr->IsIndexAddr())
        {
            *isExact   = false;
            *isNonNull = false;
            return gtGetArrayElementClassHandle(index->GetArray());
        }
        else if (GenTreeArrElem* arrElem = addr->IsArrElem())
        {
            *isExact   = false;
            *isNonNull = false;
            return gtGetArrayElementClassHandle(arrElem->GetArray());
        }
        else if (addr->OperIs(GT_ADD))
        {
            ...
            if (op2->IsIntCon() && gtIsStaticGCBaseHelperCall(op1))
            {
        ...
        else if (GenTreeIntCon* icon = addr->IsIntCon())
        {
            ...
            if ((fieldSeq != nullptr) && fieldSeq->IsField())
            {
                ...
                return gtGetFieldClassHandle(fieldHandle, isExact, isNonNull);
            }
        }
    ...
    case GT_BOX:
    {
        ...
        *isExact              = boxTempLcl->lvClassIsExact;
        *isNonNull            = true;
        return boxTempLcl->lvClassHnd;
    }
...
CORINFO_CLASS_HANDLE Compiler::gtGetHelperCallClassHandle(GenTreeCall* call, bool* isExact, bool* isNonNull)
{
    ...
    *isExact   = false;
    ...
    case CORINFO_HELP_TYPEHANDLE_TO_RUNTIMETYPE_MAYBENULL:
        ...
        *isNonNull = helper == CORINFO_HELP_TYPEHANDLE_TO_RUNTIMETYPE;
        return info.compCompHnd->getBuiltinClass(CLASSID_RUNTIME_TYPE);
...
// The following functions manage the set of temporary registers
// created by LSRA during code generation.

// There is expected to be exactly one available temporary register in the given mask
// in the temporary reg set. Get that register. No future calls to get a temporary
// register are expected. Removes the register from the set, but only in DEBUG to avoid
// doing unnecessary work in non-DEBUG builds.
RegNum GenTree::GetSingleTempReg(regMaskTP mask)
{
...
// Find the lowest number temporary register from the temporary set that is also in
// the optional given mask (typically, RBM_ALLINT or RBM_ALLFLOAT), and return it.
// Remove this register from the temporary register set, so it won't be returned again.
RegNum GenTree::ExtractTempReg(regMaskTP mask)
{
    regMaskTP availableSet = m_tempRegs & mask;
    assert(genCountBits(availableSet) >= 1);
    regMaskTP tempRegMask = genFindLowestBit(availableSet);
    m_tempRegs &= ~tempRegMask;
