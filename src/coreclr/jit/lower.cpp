...
void Lowering::InsertPutArgSplit(GenTreeCall* call, CallArgInfo* info)
{
    ...
    GenTreePutArgStk* putArgStk = NewPutArgStk(stackVal, info, call);
    ...
    if (arg->IsIntCon(0))
    {
        ...
        for (unsigned i = 0; i < info->GetRegCount(); i++)
        {
            ...
            else
            {
                ...
                regArg->SetNext(info->GetUse()->GetNext());
                info->GetUse()->SetNext(regArg);
            }
        }
    }
    else if (GenTreeFieldList* list = arg->IsFieldList())
    {
        ...
        for (unsigned i = 0; i < info->GetRegCount(); i++)
        {
            ...
            if (regVal->TypeIs(TYP_DOUBLE))
            {
                ...
                bitcast->SetRegNum(0, info->GetRegNum(i));
                bitcast->SetRegNum(1, info->GetRegNum(i + 1));
...
void Lowering::InsertFieldListPushArg(GenTreeFieldList* fields, GenTreeCall* call, CallArgInfo* argInfo)
{
    ...
    if (currentOffset != 0)
    {
        ...
        GenTreePutArgStk* putArgStk = NewPutArgStk(zero, argInfo, call);
...
void Lowering::InsertPutArg(GenTreeCall* call, CallArgInfo* info)
{
    ...
    if (info->GetRegCount() == 0)
    {
        ...
        {
#ifdef TARGET_X86
            InsertFieldListPushArg(fields, call, info);
#else
            InsertFieldListPutArgStk(fields, call, info);
#endif
...
void Lowering::LowerCallArg(GenTreeCall* call, CallArgInfo* argInfo)
{
    ...
    if (arg->TypeIs(TYP_LONG))
    {
...
#endif
        {
            ...
            GenTreePutArgStk* putArgStkLo = NewPutArgStk(argLo, argInfo, call);
            GenTreePutArgStk* putArgStkHi = NewPutArgStk(argHi, argInfo, call);
