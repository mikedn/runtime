...
bool X86Emitter::IsVexDstDstSrc(Ins ins) const
{
...
bool X86Emitter::IsVexDstSrcSrc(Ins ins) const
{
...
bool X86Emitter::IsVexTernary(Ins ins) const
{
...
bool X86Emitter::IsReallyVexTernary(Ins ins) const
{
...
bool X86Emitter::TakesVexPrefix(Ins ins) const
{
...
unsigned X86Emitter::EncodingSize(Ins ins, InsAttr size, code_t code, bool isRR)
{
...
unsigned X86Emitter::EncodingSizeR(Ins ins, InsAttr size, RegNum reg)
{
...
unsigned X86Emitter::EncodingSizeRI(Ins ins, InsAttr size, RegNum reg, ssize_t imm)
{
    if (IsShiftImm(ins))
    {
        assert(imm != 1);
...
unsigned X86Emitter::EncodingSizeRR(Ins ins, InsAttr size, RegNum reg1, RegNum reg2)
{
    if (TakesVexPrefix(ins))
    {
        code_t code;
...
unsigned X86Emitter::EncodingSizeRRI(Ins ins, InsAttr size, RegNum reg1, RegNum reg2)
{
    if (TakesVexPrefix(ins))
    {
        RegNum bReg;
...
unsigned X86Emitter::EncodingSizeRRR(Ins ins, InsAttr size, RegNum reg3)
{
    assert(TakesVexPrefix(ins));
...
unsigned X86Emitter::EncodingSizeAM(instrDesc* id, code_t code)
{
    ...
    RegNum      baseReg  = id->AddrMode().base;
    RegNum      indexReg = id->AddrMode().index;
    ...
    if (!hasDisp && BaseRegRequiresDisp(baseReg) && !BaseRegRequiresDisp(indexReg) && !IsFloatReg(indexReg))
    {
        ...
        id->AddrMode().base  = baseReg;
        id->AddrMode().index = indexReg;
    }
...
instrDesc* X86Emitter::NewInstrSmall(ssize_t imm)
{
...
instrDesc* X86Emitter::NewInstrCns(Imm32 imm)
{
    if (!instrDesc::fitsInSmallCns(imm))
    {
        instrDescCns* id = AllocInstr<instrDescCns>();
...
#ifdef TARGET_X86
instrDesc* X86Emitter::NewInstrDsp(Disp32 disp)
{
    if (disp == 0)
    {
        return NewInstr();
...
instrDesc* X86Emitter::NewInstrAMDisp(ssize_t disp)
{
    if (emitAddrMode::IsLargeDisp(disp))
    {
        instrDescAmd* id = AllocInstr<instrDescAmd>();
        id->idSetIsLargeDsp();
        INDEBUG(id->AddrMode().disp = emitAddrMode::LargeDispMarker);
        id->idaAmdVal = disp;
    ...
    instrDesc* id       = NewInstr();
    id->AddrMode().disp = disp;
    assert(id->AddrMode().disp == disp); // make sure the value fit
    return id;
}

instrDesc* X86Emitter::NewInstrAMDispImm(ssize_t disp, Imm32 imm)
{
    if (!emitAddrMode::IsLargeDisp(disp))
    {
...
void X86Emitter::Ins_Nop(unsigned size)
{
...
void X86Emitter::SetInstrAddrMode(instrDesc* id, GenTree* addr)
{
    if (!addr->isContained())
    {
        id->AddrMode().base  = addr->GetRegNum();
        id->AddrMode().index = REG_NA;
        id->AddrMode().scale = 0;
    ...
    if (GenTree* index = addrMode->GetIndex())
    {
        ...
        id->AddrMode().index = indexReg;
        id->AddrMode().scale = ScaleEncoding(addrMode->GetScale());
    }
    else
    {
        id->AddrMode().index = REG_NA;
...
void X86Emitter::Ins_A(Ins ins, InsAttr attr, GenTree* addr)
{
    if (GenTreeConstAddr* constAddr = addr->IsConstAddr())
    {
        Ins_C(ins, attr, constAddr->GetData());
        return;
    }
    ...
    if (GenTreeLclAddr* lclAddr = addr->IsLclAddr())
    {
        ...
        Ins_S(ins, attr, codeGen->GetStackAddrMode(lclAddr));
...
void X86Emitter::Ins_A_I(Ins ins, InsAttr attr, GenTree* addr, int32_t imm)
{
    AMD64_ONLY(assert(!EA_IS_CNS_RELOC(attr)));

    if (GenTreeConstAddr* constAddr = addr->IsConstAddr())
    {
        Ins_C_I(ins, attr, constAddr->GetData(), imm);
        return;
    }
...
void X86Emitter::Ins_A_R(Ins ins, InsAttr attr, GenTree* addr, RegNum reg)
{
    assert(!IsReallyVexTernary(ins));

    if (GenTreeConstAddr* constAddr = addr->IsConstAddr())
    {
        Ins_C_R(ins, attr, constAddr->GetData(), reg);
        return;
    }
...
void X86Emitter::InsRMW_A(Ins ins, InsAttr attr, GenTree* addr)
{
    instrDesc* id = NewInstrAMDisp(GetAddrModeDisp(addr));
    id->idIns(ins);
    id->idInsFmt(IF_ARW);
...
void X86Emitter::InsRMW_A_R(Ins ins, InsAttr attr, GenTree* addr, RegNum reg)
{
    instrDesc* id = NewInstrAMDisp(GetAddrModeDisp(addr));
    id->idIns(ins);
...
void X86Emitter::Ins_R_H(Ins ins, RegNum reg, void* addr DEBUGARG(HandleKind handleKind))
{
    assert(ins == INS_mov);
...
#ifdef TARGET_X86
void X86Emitter::Ins_H(Ins ins, void* addr)
{
...
void X86Emitter::InsMov_R_FS(RegNum reg, Disp32 disp)
{
...
void X86Emitter::Ins_I(Ins ins, InsAttr attr, Imm32 imm)
{
    instrDesc* id = NewInstrSmall(imm);
    id->idIns(ins);
    id->idOpSize(EA_SIZE(attr));
...
void X86Emitter::Ins_C(Ins ins, InsAttr attr, ConstData* data)
{
    instrDesc* id = NewInstr();
    id->idIns(ins);
    id->idOpSize(EA_SIZE(attr));
...
//
bool X86Emitter::IsRedundantMov(Ins ins, InsAttr size, RegNum dst, RegNum src, bool canIgnoreSideEffects)
{
    assert(IsMovIns(ins));
...
void X86Emitter::Ins_R_R_I(Ins ins, InsAttr attr, RegNum reg1, RegNum reg2, Imm32 imm)
{
    AMD64_ONLY(assert(!EA_IS_CNS_RELOC(attr)));
    ...
    Ins ins, InsAttr attr, RegNum base, RegNum index, Scale scale, Disp32 disp, RegNum reg1, RegNum reg2)
{
    ...
    id->AddrMode().base  = base;
    id->AddrMode().index = index;
    id->AddrMode().scale = ScaleEncoding(scale);
...
void X86Emitter::Ins_R_A(Ins ins, InsAttr attr, RegNum reg, GenTree* addr)
{
    assert(!HasImplicitRegPairDest(ins) && (ins != INS_imuli));
...
void X86Emitter::Ins_R_A_I(Ins ins, InsAttr attr, RegNum reg1, GenTree* addr, Imm32 imm)
{
    ...
    if (GenTreeConstAddr* constAddr = addr->IsConstAddr())
    {
        Ins_R_C_I(ins, attr, reg1, constAddr->GetData(), imm);
        return;
    }
...
void X86Emitter::Ins_R_C_I(Ins ins, InsAttr attr, RegNum reg1, ConstData* data, Imm32 imm)
{
    assert(IsSSEOrAVXOrBMIInstruction(ins) || (ins == INS_imuli));
    AMD64_ONLY(assert(!EA_IS_CNS_RELOC(attr)));
    assert(!EA_IS_GCREF_OR_BYREF(attr));
...
void X86Emitter::Ins_R_S_I(Ins ins, InsAttr attr, RegNum reg, StackAddrMode s, Imm32 imm)
{
    assert(IsSSEOrAVXOrBMIInstruction(ins) || (ins == INS_imuli));
    AMD64_ONLY(assert(!EA_IS_CNS_RELOC(attr)));
    assert(!EA_IS_GCREF_OR_BYREF(attr));
...
void X86Emitter::Ins_R_R_C(Ins ins, InsAttr attr, RegNum reg1, RegNum reg2, ConstData* data)
{
    assert(IsVexTernary(ins) && !EA_IS_GCREF_OR_BYREF(attr));
...
void X86Emitter::Ins_R_R_S(Ins ins, InsAttr attr, RegNum reg1, RegNum reg2, StackAddrMode s)
{
    assert(IsVexTernary(ins) && !EA_IS_GCREF_OR_BYREF(attr));
...
void X86Emitter::Ins_R_R_A_I(Ins ins, InsAttr attr, RegNum reg1, RegNum reg2, GenTree* addr, Imm32 imm)
{
...
void X86Emitter::Ins_R_R_C_I(Ins ins, InsAttr attr, RegNum reg1, RegNum reg2, ConstData* data, Imm32 imm)
{
    assert(IsVexTernary(ins));
    assert(!EA_IS_CNS_RELOC(attr) && !EA_IS_GCREF_OR_BYREF(attr));
    assert(IsImm8(imm));
...
void X86Emitter::Ins_R_R_R_I(Ins ins, InsAttr attr, RegNum reg1, RegNum reg2, RegNum reg3, Imm32 imm)
{
    assert(IsVexTernary(ins));
    assert(!EA_IS_CNS_RELOC(attr) && !EA_IS_GCREF_OR_BYREF(attr));
    assert(IsImm8(imm));
...
void X86Emitter::Ins_R_R_S_I(Ins ins, InsAttr attr, RegNum reg1, RegNum reg2, StackAddrMode s, Imm32 imm)
{
    assert(IsVexTernary(ins));
    assert(!EA_IS_CNS_RELOC(attr) && !EA_IS_GCREF_OR_BYREF(attr));
    assert(IsImm8(imm));
...
void X86Emitter::Ins_R_R_A_R(Ins ins, InsAttr attr, RegNum reg1, RegNum reg2, RegNum reg3, GenTree* addr)
{
    ...
    if (GenTreeConstAddr* constAddr = addr->IsConstAddr())
    {
        Ins_R_R_C_R(MapSse41BlendvToAvxBlendv(ins), attr, reg1, reg2, reg3, constAddr->GetData());
        return;
    }
...
void X86Emitter::Ins_R_R_C_R(Ins ins, InsAttr attr, RegNum reg1, RegNum reg2, RegNum reg3, ConstData* data)
{
    assert(useVexEncoding);
    assert(IsAvxBlendv(ins));
    assert(!EA_IS_CNS_RELOC(attr) && !EA_IS_GCREF_OR_BYREF(attr));
...
void X86Emitter::Ins_R_R_S_R(Ins ins, InsAttr attr, RegNum reg1, RegNum reg2, RegNum reg3, StackAddrMode s)
{
    assert(useVexEncoding);
    assert(IsAvxBlendv(ins));
    assert(!EA_IS_CNS_RELOC(attr) && !EA_IS_GCREF_OR_BYREF(attr));
...
void X86Emitter::Ins_R_R_R_R(Ins ins, InsAttr attr, RegNum reg1, RegNum reg2, RegNum reg3, RegNum reg4)
{
    assert(IsAvxBlendv(ins));
    assert(useVexEncoding);
    assert(!EA_IS_CNS_RELOC(attr) && !EA_IS_GCREF_OR_BYREF(attr));
...
void X86Emitter::Ins_C_R(Ins ins, InsAttr attr, ConstData* data, RegNum reg)
{
    X86_ONLY(noway_assert(VerifyEncodable(ins, attr, reg)));
...
void X86Emitter::Ins_C_I(Ins ins, InsAttr attr, ConstData* data, Imm32 imm)
{
    AMD64_ONLY(assert(!EA_IS_CNS_RELOC(attr)));
...
void X86Emitter::Ins_S_R_I(Ins ins, InsAttr attr, StackAddrMode s, RegNum reg, Imm32 imm)
{
    assert(reg != REG_NA);
...
void X86Emitter::Ins_A_R_I(Ins ins, InsAttr attr, GenTree* addr, RegNum reg, Imm32 imm)
{
    if (GenTreeConstAddr* constAddr = addr->IsConstAddr())
    {
        Ins_C_R_I(ins, attr, constAddr->GetData(), reg, imm);
        return;
    }
...
void X86Emitter::Ins_C_R_I(Ins ins, InsAttr attr, ConstData* data, RegNum reg, Imm32 imm)
{
    assert((ins == INS_vextracti128) || (ins == INS_vextractf128));
    assert(attr == EA_32BYTE);
    assert(reg != REG_NA);
...
void X86Emitter::VexIns_R_R_I(Ins ins, InsAttr attr, RegNum reg1, RegNum reg2, Imm32 imm)
{
...
void X86Emitter::VexIns_R_R_C(Ins ins, InsAttr attr, RegNum reg1, RegNum reg2, ConstData* data)
{
    if (useVexEncoding)
    {
        Ins_R_R_C(ins, attr, reg1, reg2, data);
    }
    else
    {
        emitIns_Mov(INS_movaps, EA_16BYTE, reg1, reg2, /* canSkip */ true);
...
void X86Emitter::VexIns_R_R_R(Ins ins, InsAttr attr, RegNum reg1, RegNum reg2, RegNum reg3)
{
    if (useVexEncoding)
    {
...
void X86Emitter::VexIns_R_R_S(Ins ins, InsAttr attr, RegNum reg1, RegNum reg2, StackAddrMode s)
{
    if (useVexEncoding)
    {
        Ins_R_R_S(ins, attr, reg1, reg2, s);
    }
    else
    {
        emitIns_Mov(INS_movaps, EA_16BYTE, reg1, reg2, /* canSkip */ true);
...
void X86Emitter::VexIns_R_R_A_I(Ins ins, InsAttr attr, RegNum reg1, RegNum reg2, GenTree* addr, Imm32 imm)
{
    if (useVexEncoding)
    {
...
void X86Emitter::VexIns_R_R_C_I(Ins ins, InsAttr attr, RegNum reg1, RegNum reg2, ConstData* data, Imm32 imm)
{
    if (useVexEncoding)
    {
        Ins_R_R_C_I(ins, attr, reg1, reg2, data, imm);
    }
    else
    {
        emitIns_Mov(INS_movaps, EA_16BYTE, reg1, reg2, /* canSkip */ true);
...
void X86Emitter::VexIns_R_R_R_I(Ins ins, InsAttr attr, RegNum reg1, RegNum reg2, RegNum reg3, Imm32 imm)
{
    if (useVexEncoding)
    {
        Ins_R_R_R_I(ins, attr, reg1, reg2, reg3, imm);
    }
    else
    {
        assert((reg3 != reg1) || (reg2 == reg1));
...
void X86Emitter::VexIns_R_R_S_I(Ins ins, InsAttr attr, RegNum reg1, RegNum reg2, StackAddrMode s, Imm32 imm)
{
    if (useVexEncoding)
    {
        Ins_R_R_S_I(ins, attr, reg1, reg2, s, imm);
    }
    else
    {
        emitIns_Mov(INS_movaps, EA_16BYTE, reg1, reg2, /* canSkip */ true);
...
void X86Emitter::Ins_S(Ins ins, InsAttr attr, StackAddrMode s)
{
    instrDesc* id = NewInstr();
    id->idIns(ins);
    id->idOpSize(EA_SIZE(attr));
...
void X86Emitter::Ins_S_I(Ins ins, InsAttr attr, StackAddrMode s, Imm32 imm)
{
    AMD64_ONLY(assert(!EA_IS_CNS_RELOC(attr)));
...
#ifdef TARGET_X86
void X86Emitter::Ins_L(Ins ins, insGroup* label)
{
...
void X86Emitter::UpdateStackLevel(Ins ins)
{
    if (ins == INS_push)
    {
...
void X86Emitter::UpdateStackLevel(Ins ins, ssize_t val)
{
...
void X86Emitter::Ins_Call(InsFmt   format,
                          void*    addr,
                          RegNum   amBase,
                          RegNum   amIndex,
                          Scale    amScale,
                          ...
                          )
{
    ...
    if (format == IF_RRD)
    {
        ...
        id->AddrMode().base  = id->idReg1();
        id->AddrMode().index = id->idReg2();
        id->idReg1(amBase);
    ...
    else if (format == IF_ARD)
    {
        ...
        id->idInsFmt(IF_ARD);
        id->AddrMode().base  = amBase;
        id->AddrMode().index = amIndex;
        id->AddrMode().scale = ScaleEncoding(amScale);
...
unsigned EmitterBase::DecodeCallGCRegs(instrDesc* id)
{
    ...
    {
        encoded = id->AddrMode().base | (id->AddrMode().index << 8);
    }
    else
    {
...
uint8_t* X86Encoder::EncodeAM(uint8_t* dst, instrDesc* id, code_t code, ssize_t* imm)
{
    ...
    else
    {
        ...
        {
            unsigned scale = id->AddrMode().scale;
