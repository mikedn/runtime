...
GenTreeCall::Use* Importer::PopCallArgs(CORINFO_SIG_INFO* sig, GenTree* extraArg)
{
    ...
    if (extraArg != nullptr)
    {
        args = comp->gtNewCallArgs(extraArg);
    }
#endif
    ...
    for (unsigned i = 0, paramCount = sig->numArgs; i < paramCount; i++)
    {
        ...
        comp->gtPrependNewCallArg(args, arg);
    }
    ...
    if (extraArg != nullptr)
    {
        comp->gtPrependNewCallArg(args, extraArg);
    }
#endif
    ...
    if (runtimeLookup.indirections == CORINFO_USEHELPER)
    {
        ...
        if (opts.IsReadyToRun())
        {
            return gtNewR2RHelperCallNode(CORINFO_HELP_READYTORUN_GENERIC_HANDLE, resolvedToken, TYP_I_IMPL,
                                          comp->gtNewCallArgs(ctxTree), &lookup->lookupKind);
        }
#endif
                                ...
                                bool*                   isSpecialIntrinsic)
{
    ...
    *pIntrinsicId = ni;
    ...
    case NI_CORINFO_INTRINSIC_Object_GetType:
        ...
        if (GenTreeBox* box = op1->IsBox())
        {
                    ...
                    comp->gtTryRemoveBoxUpstreamEffects(box, Compiler::BR_REMOVE_AND_NARROW_WANT_TYPE_HANDLE))
            {
                ...
                return gtNewHelperCallNode(CORINFO_HELP_TYPEHANDLE_TO_RUNTIMETYPE, TYP_REF, typeHandle);
            }
        }
    ...
    case NI_System_ReadOnlySpan_get_Item:
    {
        ...
        GenTree* length = comp->gtNewFieldLoad(TYP_INT, comp->gtNewFieldAddr(spanAddrUses[0], lengthHnd, lengthOffset));
        ...
        {
            indexOffset =
                comp->gtNewIconNode(static_cast<target_ssize_t>(elemSize) * indexConst->GetInt32Value(), TYP_I_IMPL);
        }
        else
        {
        ...
        GenTree* pointer = comp->gtNewFieldLoad(TYP_BYREF, comp->gtNewFieldAddr(spanAddrUses[1], ptrField, ptrOffset));
        ...
        if (GenTreeCall* call = impStackTop().val->IsCall())
        {
            if (call->IsHelperCall(CORINFO_HELP_TYPEHANDLE_TO_RUNTIMETYPE))
            {
                if (CORINFO_CLASS_HANDLE hClass = gtGetHelperArgClassHandle(call->GetFirstArg()))
                {
                    ...
                    return comp->gtNewIconNode(vm->isValueClass(hClass) &&
...
GenTree* Importer::ImportLdVirtFtn(GenTree* thisPtr, CORINFO_RESOLVED_TOKEN* resolvedToken, CORINFO_CALL_INFO* callInfo)
{
    ...
    if (opts.IsReadyToRun())
    {
        ...
        if (IsTargetAbi(CORINFO_CORERT_ABI))
        {
            ...
            return gtNewR2RHelperCallNode(CORINFO_HELP_READYTORUN_GENERIC_HANDLE, resolvedToken, TYP_I_IMPL,
                                          comp->gtNewCallArgs(ctxTree), &callInfo->codePointerLookup.lookupKind);
        }
    }
#endif
...
void Importer::ImportNewObjArray(CORINFO_RESOLVED_TOKEN* resolvedToken, const CORINFO_CALL_INFO* callInfo)
{
    ...
    else
    {
        ...
        for (int i = callInfo->sig.numArgs - 1; i >= 0; i--)
        {
            ...
            comp->gtPrependNewCallArg(args, dim);
        ...
        lastArg = comp->gtInsertNewCallArgAfter(classHandle, lastArg);
#else
        comp->gtPrependNewCallArg(args, numArgsNode);
        comp->gtPrependNewCallArg(args, classHandle);
#endif
...
GenTree* Importer::CreateClassInitTree(CORINFO_RESOLVED_TOKEN* resolvedToken)
{
    ...
    if (runtimeLookup)
    {
        return gtNewHelperCallNode(CORINFO_HELP_INITCLASS, TYP_VOID, node);
    }
                                                    ...
                                                    CORINFO_CLASS_HANDLE      structType)
{
    ...
    GenTree* addr     = gtNewHelperCallNode(fieldInfo.helper, TYP_BYREF, objPtr, fieldHnd);
                                                     ...
                                                     const CORINFO_FIELD_INFO& fieldInfo)
{
    ...
    case CORINFO_FIELD_STATIC_GENERICS_STATIC_HELPER:
    {
        ...
        addr = gtNewHelperCallNode(fieldInfo.helper, type, addr);
    }
    break;
    ...
    case CORINFO_FIELD_STATIC_SHARED_STATIC_HELPER:
    {
        ...
        if (opts.IsReadyToRun())
        {
            ...
            addr = comp->gtNewR2RHelperCallNode(CORINFO_HELP_READYTORUN_STATIC_BASE, fieldInfo.fieldLookup, TYP_BYREF);
    ...
    case CORINFO_FIELD_STATIC_READYTORUN_HELPER:
    {
        ...
        addr = comp->gtNewR2RHelperCallNode(CORINFO_HELP_READYTORUN_GENERIC_STATIC_BASE, fieldInfo.fieldLookup,
                                            TYP_BYREF, comp->gtNewCallArgs(ctxTree));
...
void Importer::impInsertHelperCall(const CORINFO_HELPER_DESC& helperInfo)
{
    ...
    {
        const CORINFO_HELPER_ARG& argInfo  = helperInfo.args[i - 1];
        GenTree*                  argValue = nullptr;
        ...
        comp->gtPrependNewCallArg(args, argValue);
    }
    ...
    impSpillNoneAppendTree(gtNewHelperCallNode(helperInfo.helperNum, TYP_VOID, args));
}
                                     ...
                                     GenTree*                newobjThis)
{
    ...
    else
    {
        ...
        case CORINFO_CALL_CODE_POINTER:
        {
            ...
            GenTree* fptr =
                impLookupToTree(resolvedToken, &callInfo->codePointerLookup, HandleKind::MethodAddr, callInfo->hMethod);
    ...
    else
    {
        ...
        if (sig->hasTypeArg())
        {
            assert(sig->getCallConv() != CORINFO_CALLCONV_VARARG);
            assert(opcode != CEE_CALLI);
        ...
        else if (sig->getCallConv() == CORINFO_CALLCONV_VARARG)
        {
...
PUSH_CALL:
    ...
    {
        // For non-candidates we must also spill, since we might have address exposed locals live
        // on the eval stack that this call can modify.
        //
    ...
    // Usual native calling conventions do not automatically widen returned small int values to INT
    // so we have to do it in the caller. R2R also follows the native calling convention and widens
    // in the caller, even if the callee is a managed method that automatically widens itself.
    ...
    DECODE_OPCODE:
        ...
        STLOC:
            ...
            if (verCurrentState.esStackDepth > 0)
            {
                ...
                else
                {
                    ...
                    else if (((valueSideEffects & GTF_GLOB_EFFECT) != GTF_NONE) || comp->impHasAddressTakenLocals(op1))
                    {
            ...
            else
            {
                ...
                else if ((lclTyp != op1->GetType()) && varTypeIsFloating(lclTyp) && varTypeIsFloating(op1->GetType()))
                {
        ...
        case CEE_LDELEMA:
...

            {
                ...
                GenTreeCall::Use* args  = comp->gtNewCallArgs(array, index, op1);
        ...
        case CEE_STELEM_REF:
...

            {
                ...
                impSpillAllAppendTree(
                    gtNewHelperCallNode(CORINFO_HELP_ARRADDR_ST, TYP_VOID, comp->gtNewCallArgs(array, index, value)));
            }
            break;
        ...
        MATH_OP2:
            ...
            {
                if ((oper == GT_MUL) && ((type == TYP_INT) || (type == TYP_LONG)))
                {
                    GenTreeIntCon* i1 = op1->IsIntCon();
                    GenTreeIntCon* i2 = op2->IsIntCon();

                    // In general IL generated by Roslyn (and hopefully any other sane compiler)
        ...
        case CEE_THROW:
            ...
            op1 = impPopStack().val;
            op1 = gtNewHelperCallNode(CORINFO_HELP_THROW, TYP_VOID, op1);
            goto POP_APPEND;
...
void Importer::ImportRefAnyVal(const uint8_t* codeAddr)
{
    ...
    {
        GenTreeCall::Use* arg1 = comp->gtNewCallArgs(op2);
        GenTreeCall::Use* arg2 = comp->gtNewCallArgs(op1);
...
void Importer::ImportUnbox(CORINFO_RESOLVED_TOKEN& resolvedToken, bool isUnboxAny)
{
    ...
    else
    {
        ...
        else
        {
            ...
            op1 = gtNewHelperCallNode(helper, TYP_STRUCT, op2, op1);
...
void Importer::ImportLdToken(const uint8_t* codeAddr)
{
    ...
    GenTreeCall* call = gtNewHelperCallNode(helper, TYP_STRUCT, token);
...
void Importer::ImportNewArr(const uint8_t* codeAddr, BasicBlock* block)
{
    ...
    if (opts.IsReadyToRun())
    {
        call = gtNewR2RHelperCallNode(CORINFO_HELP_READYTORUN_NEWARR_1, &resolvedToken, TYP_REF,
                                      comp->gtNewCallArgs(lengthOp));
...
#endif
    {
        ...
        call = gtNewHelperCallNode(helper, TYP_REF, handleOp, lengthOp);
    }
...
void Importer::ImportNewObj(const uint8_t* codeAddr, int prefixFlags, BasicBlock* block)
{
    ...
    if (!callInfo.sig.hasExplicitThis() && (newObjThis != nullptr))
    {
    ...
    if (opts.IsReadyToRun())
    {
        if (IsTargetAbi(CORINFO_CORERT_ABI))
        {
            ...
            else
            {
                ...
                {
                    helperArgs = comp->gtNewCallArgs(thisPointer, targetObjPointers);
...
GenTreeCall* Importer::gtNewHelperCallNode(CorInfoHelpFunc helper, var_types type, GenTree* arg)
{
    return comp->gtNewHelperCallNode(helper, type, comp->gtNewCallArgs(arg));
}

GenTreeCall* Importer::gtNewHelperCallNode(CorInfoHelpFunc helper, var_types type, GenTree* arg1, GenTree* arg2)
{
    return comp->gtNewHelperCallNode(helper, type, comp->gtNewCallArgs(arg1, arg2));
}
...
GenTreeHWIntrinsic* Importer::NewVecNode(var_types type, NamedIntrinsic intrinsic, var_types eltType)
{
    assert(varTypeIsTargetVec(type));
    return comp->gtNewSimdHWIntrinsicNode(type, intrinsic, eltType, varTypeSize(type));
}
...
GenTreeHWIntrinsic* Importer::NewVecNode(var_types type, NamedIntrinsic intrinsic, var_types eltType, GenTree* op1)
{
    assert(varTypeIsTargetVec(type));
    return comp->gtNewSimdHWIntrinsicNode(type, intrinsic, eltType, varTypeSize(type), op1);
}
...
{
    assert(varTypeIsTargetVec(type));
    return comp->gtNewSimdHWIntrinsicNode(type, intrinsic, eltType, varTypeSize(type), op1, op2);
}
...
{
    assert(varTypeIsTargetVec(type));
    return comp->gtNewSimdHWIntrinsicNode(type, intrinsic, eltType, varTypeSize(type), op1, op2, op3);
}
...
{
    assert(varTypeIsTargetVec(type));
    return comp->gtNewSimdHWIntrinsicNode(type, intrinsic, eltType, varTypeSize(type), numOps, ops);
}
