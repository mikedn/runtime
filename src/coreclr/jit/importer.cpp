...
GenTree* Compiler::impConvertFieldStoreValue(var_types storeType, GenTree* op2)
{
    ...
    */
    ...
    if ((storeType != op2->TypeGet()) && op2->OperIsConst() && varTypeIsIntOrI(op2->TypeGet()) &&
        varTypeIsLong(storeType))
    {
        op2 = gtNewCastNode(storeType, op2, false, storeType);
    }
#endif

#ifdef TARGET_64BIT
    // Automatic upcast for a GT_CNS_INT into TYP_I_IMPL
    if ((op2->OperGet() == GT_CNS_INT) && varTypeIsI(storeType) && !varTypeIsI(op2->gtType))
    {
    ...
    else
    {
        ...
        //
        if (varTypeIsI(op2->gtType) && (genActualType(storeType) == TYP_INT))
        {
        ...
        //
        if (varTypeIsI(storeType) && (genActualType(op2->gtType) == TYP_INT))
        {
    ...
    //
    if ((storeType != op2->TypeGet()) && varTypeIsFloating(storeType) && varTypeIsFloating(op2->gtType))
    {
        op2 = gtNewCastNode(storeType, op2, false, storeType);
    }
...
// Check target type.
            ...
            LDSFLD:
                ...
                if (compIsForInlining())
                {
                    ...
                    if ((opcode == CEE_LDSFLD) && (lclTyp == TYP_STRUCT) && (fieldInfo.structType != NO_CLASS_HANDLE))
                    {
