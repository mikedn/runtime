...
void CodeGen::EpilogGSCookieCheck(bool tailCallEpilog)
{
    ...
    if (m_gsCookieAddr == nullptr)
    {
        ...
        if (!FitsIn<int32_t>(m_gsCookieVal))
        {
            ...
            emit.Ins_S_R(INS_cmp, EA_8BYTE, regGSCheck, s);
        }
        else
#endif
        {
    ...
    else
    {
        ...
        emit.Ins_R_AR(INS_mov, EA_PTRSIZE, regGSCheck, regGSCheck, 0);
        emit.Ins_S_R(INS_cmp, EA_PTRSIZE, regGSCheck, s);
    }
...
void CodeGen::SetThrowHelperBlockStackLevel(BasicBlock* block)
{
    ...
    else
    {
        ...
        GetEmitter()->Ins_R_AR(INS_lea, EA_4BYTE, REG_SPBASE, REG_FPBASE, -genSPtoFPdelta());
    }
#endif
}
...
{
    GetEmitter()->Ins_R_R(INS_xor, size, reg, reg);
}
...
void CodeGen::instGen_Set_Reg_To_Addr(regNumber reg, void* addr DEBUGARG(void* handle) DEBUGARG(HandleKind handleKind))
{
    ...
    if (!compiler->opts.compReloc)
    {
        if (addr == nullptr)
        {
            GetEmitter()->Ins_R_R(INS_xor, EA_4BYTE, reg, reg);
        }
        else
        {
...
#endif
    {
        GetEmitter()->Ins_R_H(INS_mov, reg, addr);
    }
}
...
void CodeGen::GenIntCon(GenTreeIntCon* node)
{
    ...
    assert(!node->IsIntCon(HandleKind::TLS));
#endif
    GenIntCon(node->AsIntCon(), node->GetRegNum(), node->GetType());
    DefReg(node);
}
...
void CodeGen::GenIntCon(GenTreeIntCon* node, regNumber reg, var_types type)
{
    ...
    if (node->GetValue() == 0)
    {
        ...
        GetEmitter()->Ins_R_R(INS_xor, emitActualTypeSize(type), reg, reg);
...
{
    GenDblCon(node->AsDblCon(), node->GetRegNum(), node->GetType());
    DefReg(node);
}
...
void CodeGen::GenDblCon(GenTreeDblCon* node, regNumber reg, var_types type)
{
    ...
    {
        GetEmitter()->Ins_R_R(INS_xorps, EA_16BYTE, reg, reg);
...
{
    assert(mul->OperIs(GT_MUL, GT_OVF_SMUL, GT_OVF_UMUL) && varTypeIsIntOrI(mul->GetType()));

    bool     checkOverflow = mul->OperIs(GT_OVF_SMUL, GT_OVF_UMUL);
    ...
    if (checkOverflow)
    {
        genJumpToThrowHlpBlk(mul->OperIs(GT_OVF_UMUL) ? EJ_b : EJ_o, ThrowHelperKind::Overflow);
    }
...
void CodeGen::GenDivRem(GenTreeOp* div)
{
    ...
    {
        GetEmitter()->Ins_R_R(INS_xor, EA_4BYTE, REG_EDX, REG_EDX);
    }
    else
    {
    ...
    else
    {
        ...
        if ((ins == INS_shl) && !shift->HasImplicitFlagsDef() && (imm == 1))
        {
            if (dstReg == valueReg)
            {
                ...
                emit.Ins_R_R(INS_add, size, dstReg, valueReg);
            }
            else
            {
                // TDOO-MIKE-Review: What about SHL reg, 2/3 => LEA [reg*4/8]?
                emit.Ins_R_ARX(INS_lea, size, dstReg, valueReg, valueReg, 1, 0);
            }
        }
...
void CodeGen::GenAddSubBitwise(GenTreeOp* node)
{
    ...
    {
        if (node->OperIs(GT_ADD) && !node->HasImplicitFlagsDef() && (op2->IsContainedIntCon() || (reg2 != REG_NA)))
        {
            if (GenTreeIntCon* imm = op2->IsContainedIntCon())
            {
                emit.Ins_R_AR(INS_lea, attr, dstReg, reg1, imm->GetInt32Value());
            }
            else
            {
                emit.Ins_R_ARX(INS_lea, attr, dstReg, reg1, reg2, 1, 0);
            }
    ...
    if (node->IsOverflowOp())
    {
        ...
        genJumpToThrowHlpBlk(node->OperIs(GT_OVF_UADD, GT_OVF_USUB
#ifndef TARGET_64BIT
                                          ,
                                          GT_OVF_UADDC, GT_OVF_USUBB
#endif
                                          )
                                 ? EJ_b
                                 : EJ_o,
                             ThrowHelperKind::Overflow);
    }
...
void CodeGen::GenFloatReturn(GenTree* src)
{
    ...
    if (IsRegCandidateLclLoad(src) && src->AsLclLoad()->GetLcl()->lvOnFrame)
    {
        ...
        if (srcLcl->GetRegNum() != REG_STK)
        {
            ...
            GetEmitter()->Ins_S_R(ins_Store(srcType), srcSize, srcReg, s);
        }

        GetEmitter()->Ins_S(INS_fld, srcSize, s);
    }
    else
    {
...
void CodeGen::PrologEstablishFramePointer(int delta, bool reportUnwindData)
{
    ...
    {
        GetEmitter()->Ins_R_AR(INS_lea, EA_PTRSIZE, REG_FPBASE, REG_ESP, delta);
...
{
    GetEmitter()->Ins_AR_R(INS_test, EA_4BYTE, REG_SPBASE, REG_SPBASE, 0);
...
target_ssize_t CodeGen::genStackPointerConstantAdjustmentLoopWithProbe(ssize_t spDelta, regNumber regTmp)
{
    ...
    if ((lastTouchDelta == 0) || (lastTouchDelta + STACK_PROBE_BOUNDARY_THRESHOLD_BYTES > pageSize))
    {
        ...
        GetEmitter()->Ins_AR_R(INS_test, EA_PTRSIZE, REG_EAX, REG_SPBASE, 0);
        lastTouchDelta = 0;
    }
...
void CodeGen::genStackPointerDynamicAdjustmentWithProbe(regNumber regSpDelta, regNumber regTmp)
{
    ...
    emit.Ins_AR_R(INS_test, EA_4BYTE, REG_SPBASE, REG_SPBASE, 0);
...
ALLOC_DONE:
    ...
    emit.Ins_R_AR(INS_lea, EA_PTRSIZE, targetReg, REG_SPBASE, (int)locAllocStackOffset);
...
void CodeGen::GenStructStoreUnrollInit(GenTree* store, ClassLayout* layout)
{
    ...
    if (size == 1)
    {
        ...
        {
            emit.Ins_ARX_I(INS_mov, EA_1BYTE, dstAddrBaseReg, dstAddrIndexReg, dstAddrIndexScale, dstOffset, 0);
        }
            ...
            )
    {
        ...
        if (src->gtSkipReloadOrCopy()->IsIntCon(0))
        {
            ...
            emit.Ins_R_R(INS_xorps, EA_16BYTE, srcXmmReg, srcXmmReg);
        }
        else
        {
            emit.emitIns_Mov(INS_movd, EA_PTRSIZE, srcXmmReg, srcIntReg, /* canSkip */ false);
            emit.Ins_R_R(INS_punpckldq, EA_16BYTE, srcXmmReg, srcXmmReg);
#ifdef TARGET_X86
        ...
        for (; size >= minSize; size -= regSize, dstOffset += regSize)
        {
            ...
            if (dstLcl != nullptr)
            {
                emit.Ins_S_R(xmmMov, EA_ATTR(regSize), srcXmmReg, GetStackAddrMode(dstLcl, dstOffset));
            }
            else
            {
                emit.Ins_ARX_R(xmmMov, EA_ATTR(regSize), srcXmmReg, dstAddrBaseReg, dstAddrIndexReg, dstAddrIndexScale,
                               dstOffset);
            }
        }
    }
    ...
    for (unsigned regSize = REGSIZE_BYTES; size > 0; size -= regSize, dstOffset += regSize)
    {
        ...
        if (dstLcl != nullptr)
        {
            emit.Ins_S_R(INS_mov, EA_ATTR(regSize), srcIntReg, GetStackAddrMode(dstLcl, dstOffset));
        }
        else
        {
            emit.Ins_ARX_R(INS_mov, EA_ATTR(regSize), srcIntReg, dstAddrBaseReg, dstAddrIndexReg, dstAddrIndexScale,
                           dstOffset);
        }
    }
}
...
void CodeGen::GenStructStoreUnrollCopy(GenTree* store, ClassLayout* layout)
{
    ...
    if (size >= XMM_REGSIZE_BYTES)
    {
        RegNum tempReg = store->GetSingleTempReg(RBM_ALLFLOAT);
             ...
             size -= regSize, srcOffset += regSize, dstOffset += regSize)
        {
            if (srcLcl != nullptr)
            {
                emit.Ins_R_S(INS_movups, EA_ATTR(regSize), tempReg, GetStackAddrMode(srcLcl, srcOffset));
            }
            else
            {
                emit.Ins_R_ARX(INS_movups, EA_ATTR(regSize), tempReg, srcAddrBaseReg, srcAddrIndexReg,
                               srcAddrIndexScale, srcOffset);
            }

            if (dstLcl != nullptr)
            {
                emit.Ins_S_R(INS_movups, EA_ATTR(regSize), tempReg, GetStackAddrMode(dstLcl, dstOffset));
            }
            else
            {
                emit.Ins_ARX_R(INS_movups, EA_ATTR(regSize), tempReg, dstAddrBaseReg, dstAddrIndexReg,
    ...
    if (size > 0)
    {
        RegNum tempReg = store->GetSingleTempReg(RBM_ALLINT);
        ...
        for (unsigned regSize = REGSIZE_BYTES; size > 0; size -= regSize, srcOffset += regSize, dstOffset += regSize)
        {
            ...
            if (srcLcl != nullptr)
            {
                emit.Ins_R_S(INS_mov, EA_ATTR(regSize), tempReg, GetStackAddrMode(srcLcl, srcOffset));
            }
            else
            {
                emit.Ins_R_ARX(INS_mov, EA_ATTR(regSize), tempReg, srcAddrBaseReg, srcAddrIndexReg, srcAddrIndexScale,
                               srcOffset);
            }

            if (dstLcl != nullptr)
            {
                emit.Ins_S_R(INS_mov, EA_ATTR(regSize), tempReg, GetStackAddrMode(dstLcl, dstOffset));
            }
            else
            {
                emit.Ins_ARX_R(INS_mov, EA_ATTR(regSize), tempReg, dstAddrBaseReg, dstAddrIndexReg, dstAddrIndexScale,
                               dstOffset);
            }
        }
    }
...
void CodeGen::GenStructStoreUnrollRegs(GenTree* store, ClassLayout* layout)
{
    ...
    for (; (regIndex < regCount) && (size > 0); regIndex++, dstOffset += regSize, size -= regSize)
    {
        ...
        if (dstLcl != nullptr)
        {
            emit.Ins_S_R(ins, attr, reg, GetStackAddrMode(dstLcl, dstOffset));
        }
        else
        {
            emit.Ins_ARX_R(ins, attr, reg, dstAddrBaseReg, dstAddrIndexReg, dstAddrIndexScale, dstOffset);
        }
    }
    ...
    if ((regIndex < regCount) && (size > 0))
    {
        ...
        for (unsigned regShift = 0; size > 0; regShift = regSize, dstOffset += regSize, size -= regSize)
        {
            ...
            if (dstLcl != nullptr)
            {
                emit.Ins_S_R(INS_mov, attr, reg, GetStackAddrMode(dstLcl, dstOffset));
            }
            else
            {
                emit.Ins_ARX_R(INS_mov, attr, reg, dstAddrBaseReg, dstAddrIndexReg, dstAddrIndexScale, dstOffset);
            }
        }
    }
}
...
void CodeGen::GenStructStoreUnrollRegsWB(GenTreeIndStoreObj* store)
{
    ...
    if (layout->IsGCRef(0))
    {
        ...
        emit.Ins_R_AR(INS_lea, emitTypeSize(addr->GetType()), REG_ARG_0, addrReg, addrOffset);
    ...
    {
        emit.Ins_AR_R(INS_mov, EA_8BYTE, valReg0, addrReg, addrOffset);
    }
    ...
    {
        emit.Ins_AR_R(INS_mov, EA_8BYTE, valReg1, addrReg, addrOffset);
    }
}
...
void CodeGen::GenSwitchTable(GenTreeOp* node)
{
    ...
    {
        emit.Ins_ARX(INS_i_jmp, EA_4BYTE, baseReg, indexReg, 4, 0);
    }
#else
    emit.Ins_R_ARX(INS_mov, EA_4BYTE, baseReg, baseReg, indexReg, 4, 0);
...
void CodeGen::GenLockAdd(GenTreeOp* node)
{
    ...
    {
        GetEmitter()->Ins_ARX_I(INS_add, size, addrReg, REG_NA, 0, 0, imm->GetInt32Value());
    }
    else
    {
        GetEmitter()->Ins_AR_R(INS_add, size, valueReg, addrReg, 0);
    }
}
...
void CodeGen::GenInterlocked(GenTreeOp* node)
{
    ...
    GetEmitter()->Ins_AR_R(ins, size, destReg, addrReg, 0);
    DefReg(node);
}
...
void CodeGen::GenCmpXchg(GenTreeCmpXchg* node)
{
    ...
    GetEmitter()->Ins_Lock();
    GetEmitter()->Ins_AR_R(INS_cmpxchg, size, valueReg, addrReg, 0);
...
void CodeGen::GenMemoryBarrier(GenTree* barrier)
{
    ...
    {
        GetEmitter()->Ins_Lock();
        GetEmitter()->Ins_ARX_I(INS_or, EA_4BYTE, REG_SPBASE, REG_NA, 0, 0, 0);
    }
}

void CodeGen::GenBoundsCheck(GenTreeBoundsChk* bndsChk)
{
...
void CodeGen::GenNullCheck(GenTreeNullCheck* node)
{
    ...
    GetEmitter()->Ins_AR_R(INS_cmp, EA_4BYTE, reg, reg, 0);
}
...
void CodeGen::GenLclLoad(GenTreeLclLoad* load)
{
...
#endif
    {
        ...
        GetEmitter()->Ins_R_S(ins_Load(type, IsSimdLocalAligned(lcl)), emitTypeSize(type), load->GetRegNum(),
...
void CodeGen::GenLclLoadFld(GenTreeLclLoadFld* load)
{
...
#endif
    {
        ...
        GetEmitter()->Ins_R_S(ins_Load(type), emitTypeSize(type), load->GetRegNum(), GetStackAddrMode(load));
    }
...
void CodeGen::GenLclStoreFld(GenTreeLclStoreFld* store)
{
    ...
    else if (!src->isContained())
    {
        ...
        emit.Ins_S_R(ins_Store(type), emitTypeSize(type), UseReg(src), GetStackAddrMode(store));
    }
    ...
    {
        emit.Ins_S_R(ins_Store(type), emitTypeSize(type), src->AsRegUse()->GetSrcRegNum(), GetStackAddrMode(store));
    }
...
void CodeGen::GenLclStore(GenTreeLclStore* store)
{
...
#ifdef DEBUG
    {
        ...
        assert(!varTypeUsesFloatReg(lclRegType) || (emitTypeSize(lclRegType) == emitTypeSize(srcRegType)) ||
               (varTypeIsVec(lclRegType) && (varTypeTargetVec(lclRegType) == varTypeTargetVec(srcRegType))));
    }
#endif
...
#endif
        {
            ...
            if (src->isContained())
            {
                if (src->OperIs(GT_BITCAST))
                {
                    ...
                    GetEmitter()->Ins_S_R(ins, attr, bitCastSrcReg, s);
                }
                else if (src->OperIsRMWMemOp())
                {
            ...
            else
            {
                ...
                GetEmitter()->Ins_S_R(ins, attr, srcReg, s);
            }
        }
...
void CodeGen::GenLclStoreMultiRegVectorReg(GenTreeLclStore* store)
{
    ...
    if (dstReg == srcReg0)
    {
        emit.Ins_R_R(INS_movlhps, EA_16BYTE, dstReg, srcReg1);
    }
    else if (UseVexEncoding())
    {
        emit.Ins_R_R_R(INS_unpcklpd, EA_16BYTE, dstReg, srcReg0, srcReg1);
    }
...
void CodeGen::GenLclStoreMultiRegVectorMem(GenTreeLclStore* store)
{
    ...
    emit.Ins_S_R(INS_movsd, EA_8BYTE, reg0, GetStackAddrMode(lcl, 0));
    emit.Ins_S_R(store->TypeIs(TYP_SIMD12) ? INS_movss : INS_movsd, store->TypeIs(TYP_SIMD12) ? EA_4BYTE : EA_8BYTE,
...
void CodeGen::GenLclStoreMultiRegVectorReg(GenTreeLclStore* store)
{
    ...
    emit.Ins_R_R(INS_unpcklps, EA_16BYTE, dstReg, tmpReg);
...
void CodeGen::GenLclStoreMultiRegVectorMem(GenTreeLclStore* store)
{
    ...
    emit.Ins_S_R(INS_mov, EA_4BYTE, reg0, GetStackAddrMode(lcl, 0));
    emit.Ins_S_R(INS_mov, EA_4BYTE, reg1, GetStackAddrMode(lcl, 4));
...
void CodeGen::GenStoreLclRMW(var_types type, StackAddrMode s, GenTree* src)
{
    ...
    instruction ins  = GetOperIns(src->GetOper());
    emitAttr    attr = emitTypeSize(type);
    ...
    if (!src->isUsedFromReg())
    {
        ...
        if (isShift && (imm == 1))
        {
            GetEmitter()->Ins_S(MapShiftInsToShiftBy1Ins(ins), attr, s);
        }
        else
        {
            if (isShift)
...
void CodeGen::GenIndexAddr(GenTreeIndexAddr* node)
{
    ...
    if ((node->gtFlags & GTF_INX_RNGCHK) != 0)
    {
        ...
        {
            GetEmitter()->Ins_R_AR(INS_mov, EA_4BYTE, tmpReg, baseReg, node->GetLenOffs());
            GetEmitter()->Ins_R_R(INS_cmp, EA_8BYTE, indexReg, tmpReg);
        }
        else
#endif // TARGET_64BIT
        {
            GetEmitter()->Ins_R_AR(INS_cmp, EA_4BYTE, indexReg, baseReg, node->GetLenOffs());
        }
...
void CodeGen::GenIndStore(GenTreeIndStore* store)
{
    ...
    if (value->OperIs(GT_ADD) && (src->IsIntCon(1) || src->IsIntCon(-1)))
    {
        ...
        emit.InsRMW_A(src->IsIntCon(1) ? INS_inc : INS_dec, attr, addr);
    }
    ...
    {
        emit.InsRMW_A_I(GetOperIns(value->GetOper()), attr, addr, imm->GetInt32Value());
    }
    else
    {
        emit.InsRMW_A_R(GetOperIns(value->GetOper()), attr, addr, src->GetRegNum());
    }
}
...
void CodeGen::GenIndStoreRMWShift(GenTree* addr, GenTreeOp* shift, GenTree* shiftBy)
{
    ...
    if (shiftBy->isUsedFromReg())
    {
        ...
        emit.InsRMW_A(ins, attr, addr);
    }
    else if (shiftBy->AsIntCon()->GetInt32Value() == 1)
    {
        emit.InsRMW_A(MapShiftInsToShiftBy1Ins(ins), attr, addr);
    }
    else if (GenTreeIntCon* imm = shiftBy->IsContainedIntCon())
    {
        emit.InsRMW_A_I(MapShiftInsToShiftByImmIns(ins), attr, addr, imm->GetInt32Value());
    }
    else
    {
        emit.InsRMW_A_R(MapShiftInsToShiftByImmIns(ins), attr, addr, shiftBy->GetRegNum());
    }
}
...
void CodeGen::GenRegSwap(GenTreeOp* tree)
{
    ...
    GetEmitter()->Ins_R_R(INS_xchg, size, reg1, reg2);
...
void CodeGen::GenCall(GenTreeCall* call)
{
    ...
    if (call->HasNullCheck())
    {
        ...
        emit.Ins_AR_R(INS_cmp, EA_4BYTE, REG_ARG_0, REG_ARG_0, 0);
    }
    ...
    if ((compiler->lvaCallSpCheckLcl != nullptr) && call->IsUserCall())
    {
        ...
        assert(lcl->lvOnFrame && lcl->lvDoNotEnregister);
        emit.Ins_S_R(INS_mov, EA_4BYTE, REG_SPBASE, GetStackAddrMode(lcl, 0));
    }
#endif
    ...
    emitAttr retReg1Attr = EA_UNKNOWN;

    if (call->HasMultiRegRetVal())
    {
        retReg0Attr = emitTypeSize(call->GetRegType(0));
        retReg1Attr = emitTypeSize(call->GetRegType(1));
    }
    ...
    emit.Ins_Call(
        ...
        false,
        retReg0Attr UNIX_AMD64_ABI_ONLY_ARG(retReg1Attr),
#ifdef TARGET_X86
        argSizeForEmitter,
#endif
    ...
    if ((compiler->lvaCallSpCheckLcl != nullptr) && call->IsUserCall())
    {
        ...
        if (!call->CallerPop() && (stackArgBytes != 0))
        {
            ...
            emit.Ins_S_R(INS_cmp, EA_4BYTE, REG_ARG_0, GetStackAddrMode(compiler->lvaCallSpCheckLcl, 0));
            spRegCheck = REG_ARG_0;
        }

        insGroup* spCheckEndLabel = emit.CreateTempLabel();
        emit.Ins_S_R(INS_cmp, EA_4BYTE, spRegCheck, GetStackAddrMode(compiler->lvaCallSpCheckLcl, 0));
...
void CodeGen::GenJmp(GenTreeJmp* jmp)
{
    ...
    for (LclVarDsc* lcl : compiler->Params())
    {
        ...
        if (varTypeIsStruct(type))
        {
            ...
            if (layout->GetSysVAmd64AbiRegCount() > 1)
            {
                ...
                GetEmitter()->Ins_R_S(ins_Load(type), emitTypeSize(type), reg, GetStackAddrMode(lcl, 8));
                liveness.AddLiveLclRegs(genRegMask(reg));
                liveness.SetGCRegType(reg, type);
            }
    ...
    for (LclVarDsc* lcl : compiler->Params())
    {
        if (lcl->IsRegParam())
        {
            ...
            if (IsFloatReg(reg))
            {
                RegNum intReg = MapVarargsParamFloatRegToIntReg(reg);
    ...
    {
        RegNum reg = intArgRegs[i];

        if ((varargsIntRegMask & genRegMask(reg)) != RBM_NONE)
        {
            GetEmitter()->Ins_R_S(INS_mov, EA_8BYTE, reg, GetStackAddrMode(0u, i * REGSIZE_BYTES));
...
void CodeGen::GenLea(GenTreeAddrMode* lea)
{
    ...
    GetEmitter()->Ins_R_ARX(INS_lea, emitTypeSize(lea->GetType()), dstReg, baseReg, indexReg, scale, offset);
...
void CodeGen::GenOvfUnsigned(GenTreeUnOp* node)
{
    ...
    GetEmitter()->Ins_R_R(INS_test, size, srcReg, srcReg);
    genJumpToThrowHlpBlk(EJ_l, ThrowHelperKind::Overflow);
...
void CodeGen::GenIntToFloat(GenTreeUnOp* cast)
{
    ...
    // IND_LOAD or other memory node and in that case the source must not be contained.
    ...
    emit.Ins_R_R(INS_xorps, EA_16BYTE, dstReg, dstReg);
    ...
    if ((srcType == TYP_LONG) && cast->OperIs(GT_UTOF))
    {
        ...
        if (dstType == TYP_DOUBLE)
        {
            ...
            {
                u8ToDblBitmask = emit.GetFloatConst(jitstd::bit_cast<double>(0x43f0000000000000ULL), TYP_DOUBLE);
            }
        ...
        else
        {
            ...
            {
                u8ToFltBitmask = emit.GetFloatConst(jitstd::bit_cast<float>(0x5f800000U), TYP_FLOAT);
            }
...
#ifdef TARGET_64BIT
    assert(!cast->OperIs(GT_FTOU) || cast->TypeIs(TYP_INT));
#else
...
void CodeGen::GenCkfinite(GenTree* node)
{
    ...
    int       expMask = type == TYP_FLOAT ? 0x7F800000 : 0x7FF00000; // Bit mask to extract exponent.
    RegNum    dstReg  = node->GetRegNum();
    ...
    GetEmitter()->emitIns_Mov(INS_movd, emitTypeSize(type), tmpReg, srcReg, /* canSkip */ false);
...
void CodeGen::GenBitCast(GenTreeUnOp* bitcast)
{
    ...
    if (src->OperIs(GT_LONG))
    {
        ...
        emit.emitIns_Mov(INS_movd, EA_4BYTE, dstReg, srcReg0, /*canSkip*/ false);
        ...
        else
        {
            ...
            emit.emitIns_Mov(INS_movd, EA_4BYTE, tmpReg, srcReg1, /*canSkip*/ false);
            emit.Ins_R_R(INS_punpckldq, EA_16BYTE, dstReg, tmpReg);
        }
    ...
    if (dstType == TYP_LONG)
    {
        assert(src->TypeIs(TYP_DOUBLE, TYP_SIMD8));
        ...
        {
            RegNum srcReg = UseReg(src);
            emit.emitIns_Mov(INS_movd, EA_4BYTE, dstRegs[0], srcReg, /*canSkip*/ false);
        ...
        else
        {
            ...
            emit.Ins_R_S(INS_mov, EA_4BYTE, dstRegs[0], {src->AsLclLoad()->GetLcl(), 0});
            emit.Ins_R_S(INS_mov, EA_4BYTE, dstRegs[1], {src->AsLclLoad()->GetLcl(), 4});
        }
    ...
    if (!src->isUsedFromReg())
    {
        ...
        emit.Ins_R_S(ins, emitTypeSize(dstType), dstReg, GetStackAddrMode(lcl, 0));
    }
    else
    {
        RegNum srcReg = UseReg(src);
...
void CodeGen::AlignStackBeforeCall(GenTreeArgStore* store)
{
#ifdef UNIX_X86_ABI
    AlignStackBeforeCall(store->GetCall());
#endif
}
...
void CodeGen::PreAdjustStackForArgStore(unsigned argSize)
{
    // If argSize is large, we need to probe the stack like we do in the prolog (PrologAllocLclFrame)
    // or for localloc (GenLclAlloc), to ensure we touch the stack pages sequentially, and don't miss
...
void CodeGen::GenArgStore(GenTreeArgStore* store)
{
    ...
    if (varTypeIsSmall(type))
    {
        ...
        if (src->isUsedFromReg())
        {
            ...
            emit.Ins_AR_R(INS_mov, emitTypeSize(type), srcReg, REG_SPBASE, store->GetOffset());
        }
        else
        {
            emit.Ins_ARX_I(INS_mov, emitTypeSize(type), REG_SPBASE, REG_NA, 1, store->GetOffset(),
    ...
    if ((store->GetPushSize() > varTypeSize(type)) && !varTypeIsSIMD(src->GetType()))
    {
    ...
    if (!src->isUsedFromReg())
    {
        ...
        {
            emit.Ins_S(INS_push, emitTypeSize(type), s);
        }
        else if (src->OperIs(GT_IND_LOAD))
        {
            emit.Ins_A(INS_push, emitTypeSize(type), src->AsIndLoad()->GetAddr());
        }
        else if (GenTreeIntCon* handle = src->IsIntConHandle())
        {
            emit.Ins_H(INS_push, handle->GetAddr());
        }
        else
        {
    ...
    if (varTypeIsSIMD(type))
    {
        ...
        {
            emit.Ins_AR_R(ins_Store(type), EA_ATTR(size), srcReg, REG_SPBASE, 0);
        }
...
void CodeGen::GenStructArgStore(GenTreeArgStore* store)
{
    ...
    if (src->IsIntCon(0))
    {
        ...
        {
            assert(store->HasAllTempRegs(RBM_ECX | RBM_EDI));
        ...
        else
        {
            ...
            RegNum zeroXmmReg = store->GetSingleTempReg(RBM_ALLFLOAT);
            ...
            {
                emit.Ins_AR_R(INS_movups, EA_16BYTE, zeroXmmReg, REG_SPBASE, offset);
                offset += XMM_REGSIZE_BYTES;
            }

            if (size - offset >= 8)
            {
                emit.Ins_AR_R(INS_movq, EA_8BYTE, zeroXmmReg, REG_SPBASE, offset);
                offset += 8;
            }
            ...
            if (size - offset != 0)
            {
                ...
                emit.Ins_AR_R(INS_movd, EA_4BYTE, zeroXmmReg, REG_SPBASE, offset);
            }
        }
    ...
    if (store->GetKind() == GenTreeArgStore::Kind::Unroll)
    {
        assert(!layout->HasGCPtr());
        ...
        {
            xmmTmpReg = store->GetSingleTempReg(RBM_ALLFLOAT);
        }

        if ((size % (XMM_REGSIZE_BYTES / 2)) != 0)
        {
            intTmpReg = store->GetSingleTempReg(RBM_ALLINT);
        }
        ...
        if ((size == 1) || (size == 2) || (size == 4) || (size == 12))
        {
            ...
            if ((size == 1) || (size == 2))
            {
                if (srcLcl != nullptr)
                {
                    emit.Ins_R_S(INS_movzx, EA_ATTR(size), intTmpReg, GetStackAddrMode(srcLcl, srcOffset));
                }
                else
                {
                    emit.Ins_R_ARX(INS_movzx, EA_ATTR(size), intTmpReg, srcAddrBaseReg, srcAddrIndexReg,
                                   srcAddrIndexScale, srcOffset);
                }
            ...
            else if ((size == 4) || (size == 12))
            {
                if (srcLcl != nullptr)
                {
                    emit.Ins_S(INS_push, EA_4BYTE, GetStackAddrMode(srcLcl, srcOffset + (size & 8)));
                }
                else
                {
                    emit.Ins_ARX(INS_push, EA_4BYTE, srcAddrBaseReg, srcAddrIndexReg, srcAddrIndexScale,
                                 srcOffset + (size & 8));
                }
            }
            ...
            if (size == 12)
            {
                if (srcLcl != nullptr)
                {
                    emit.Ins_R_S(INS_movq, EA_8BYTE, xmmTmpReg, GetStackAddrMode(srcLcl, srcOffset));
                }
                else
                {
                    emit.Ins_R_ARX(INS_movq, EA_8BYTE, xmmTmpReg, srcAddrBaseReg, srcAddrIndexReg, srcAddrIndexScale,
                                   srcOffset);
                }
        ...
        for (unsigned regSize = XMM_REGSIZE_BYTES, offset = 0; size != 0; size -= regSize, offset += regSize)
        {
            ...
            if (srcLcl != nullptr)
            {
                emit.Ins_R_S(ins, EA_ATTR(regSize), tmpReg, GetStackAddrMode(srcLcl, srcOffset + offset));
            }
            else
            {
                emit.Ins_R_ARX(ins, EA_ATTR(regSize), tmpReg, srcAddrBaseReg, srcAddrIndexReg, srcAddrIndexScale,
                               srcOffset + offset);
            }

            emit.Ins_AR_R(ins, EA_ATTR(regSize), tmpReg, REG_SPBASE, offset);
        }

        return;
    }

    if (store->GetKind() == GenTreeArgStore::Kind::Push)
    {
        ...
        assert((srcLcl != nullptr) || (layout->GetSize() % REGSIZE_BYTES == 0));

        for (int i = store->GetPushSize() / REGSIZE_BYTES - 1; i >= 0; --i)
        {
            ...
            if (srcLcl != nullptr)
            {
                emit.Ins_S(INS_push, slotAttr, GetStackAddrMode(srcLcl, slotSrcOffset));
            }
            else
            {
                emit.Ins_ARX(INS_push, slotAttr, srcAddrBaseReg, srcAddrIndexReg, srcAddrIndexScale, slotSrcOffset);
            }
    ...
    assert(store->HasAllTempRegs(RBM_RSI | RBM_RDI | RBM_RCX));
    ...
    if (srcLcl != nullptr)
    {
        emit.Ins_R_S(INS_lea, EA_PTRSIZE, REG_RSI, GetStackAddrMode(srcLcl, srcOffset));
    }
    else if ((srcAddrIndexReg != REG_NA) || (srcOffset != 0))
    {
        emit.Ins_R_ARX(INS_lea, srcAddrAttr, REG_RSI, srcAddrBaseReg, srcAddrIndexReg, srcAddrIndexScale, srcOffset);
    }
    else
    {
        emit.emitIns_Mov(INS_mov, srcAddrAttr, REG_RSI, srcAddrBaseReg, /* canSkip */ false);
    }
...
void CodeGen::GenArgStore(GenTreeArgStore* store)
{
    ...
    {
        RegNum srcReg = UseReg(src);
        emit.Ins_S_R(ins_Store(type), emitTypeSize(type), srcReg, {argLclNum, argLclOffs});
    }
    else
    {
        assert(src->IsContainedIntCon());
...
void CodeGen::GenStructArgStore(GenTreeArgStore* store, unsigned argLclNum DEBUGARG(unsigned argLclSize))
{
    ...
    if (src->IsIntCon(0))
    {
        ...
        RegNum srcReg = UseReg(src);
        emit.Ins_S_R(INS_mov, size > 4 ? EA_8BYTE : EA_4BYTE, srcReg, {argLclNum, argLclOffs});
#elif defined(UNIX_AMD64_ABI)
        size     = roundUp(size, REGSIZE_BYTES);

        if (store->GetKind() == GenTreeArgStore::Kind::RepInstrZero)
        {
            assert(store->HasAllTempRegs(RBM_RCX | RBM_RDI));

            RegNum srcReg = UseReg(src);
            emit.emitIns_Mov(INS_mov, EA_8BYTE, REG_RAX, srcReg, /*canSkip*/ true);
            emit.Ins_R_S(INS_lea, EA_8BYTE, REG_RDI, {argLclNum, argLclOffs});
        ...
        else
        {
            ...
            while (offset + XMM_REGSIZE_BYTES <= size)
            {
                emit.Ins_S_R(INS_movups, EA_16BYTE, zeroXmmReg, {argLclNum, argLclOffs + offset});
                offset += XMM_REGSIZE_BYTES;
            }
            ...
            if (size - offset >= 8)
            {
                ...
                emit.Ins_S_R(INS_movq, EA_8BYTE, zeroXmmReg, {argLclNum, argLclOffs + offset});
            }
        }
#endif
    ...
    if (store->GetKind() == GenTreeArgStore::Kind::Unroll)
    {
        ...
        for (unsigned regSize = XMM_REGSIZE_BYTES, offset = 0; size != 0; size -= regSize, offset += regSize)
        {
            ...
            emit.Ins_S_R(ins, EA_ATTR(regSize), tmpReg, GetStackAddrMode(argLclNum, argLclOffs + offset));
        }
        return;
    }
    ...
    if ((store->GetKind() == GenTreeArgStore::Kind::RepInstr) ||
        (store->GetKind() == GenTreeArgStore::Kind::RepInstrXMM))
    {
        assert(store->HasAllTempRegs(RBM_RSI | RBM_RDI | RBM_RCX));

        emit.Ins_R_S(INS_lea, EA_PTRSIZE, REG_RDI, GetStackAddrMode(argLclNum, argLclOffs));
        ...
        {
            emit.Ins_R_ARX(INS_lea, srcAddrAttr, REG_RSI, srcAddrBaseReg, srcAddrIndexReg, srcAddrIndexScale,
                           srcOffset);
        }
        else
        {
    ...
    {
        assert((store->GetKind() == GenTreeArgStore::Kind::GCUnroll) ||
               (store->GetKind() == GenTreeArgStore::Kind::GCUnrollXMM));

        intTmpReg = store->GetSingleTempReg(RBM_ALLINT);
    }

    if ((store->GetKind() == GenTreeArgStore::Kind::RepInstrXMM) ||
        (store->GetKind() == GenTreeArgStore::Kind::GCUnrollXMM))
    {
        xmmTmpReg = store->GetSingleTempReg(RBM_ALLFLOAT);
    }
    ...
    ClassLayout* argLayout = compiler->typGetLayoutByNum(store->GetArgTypeNum());
    ...
    for (unsigned i = 0; i < numSlots; i++)
    {
        ...
        while ((i + nonGCSequenceLength < numSlots) && !layout->IsGCPtr(i + nonGCSequenceLength))
        {
            nonGCSequenceLength++;
        }
        ...
        if (nonGCSequenceLength <= 1)
        {
            ...
            if (srcLcl != nullptr)
            {
                emit.Ins_R_S(INS_mov, slotAttr, intTmpReg, GetStackAddrMode(srcLcl, srcOffset));
            }
            else
            {
                emit.Ins_R_ARX(INS_mov, slotAttr, intTmpReg, srcAddrBaseReg, srcAddrIndexReg, srcAddrIndexScale,
                               srcOffset);
            }
            emit.Ins_S_R(INS_mov, slotAttr, intTmpReg, GetStackAddrMode(argLclNum, argLclOffs + i * REGSIZE_BYTES));
        ...
        if (nonGCSequenceLength == 2)
        {
            ...
            if (srcLcl != nullptr)
            {
                emit.Ins_R_S(INS_movups, EA_16BYTE, xmmTmpReg, GetStackAddrMode(srcLcl, srcOffset));
            }
            else
            {
                emit.Ins_R_ARX(INS_movups, EA_16BYTE, xmmTmpReg, srcAddrBaseReg, srcAddrIndexReg, srcAddrIndexScale,
                               srcOffset);
            }

            emit.Ins_S_R(INS_movups, EA_16BYTE, xmmTmpReg, GetStackAddrMode(argLclNum, argLclOffs + i * REGSIZE_BYTES));
        ...
        if (nonGCSequenceLength < CPOBJ_NONGC_SLOTS_LIMIT)
        {
            ...
            {
                emit.emitIns(INS_movs, EA_PTRSIZE);
            }
        }
        else
        {
...
#ifdef TARGET_X86
void CodeGen::PushReg(var_types type, RegNum srcReg)
{
...
// for x86 stack unwinding
    ...
    {
        GetEmitter()->Ins_H(INS_push, compiler->opts.compProfilerMethHnd);
    }
    else
    {
...
void CodeGen::inst_R_AM_I(instruction ins, emitAttr attr, RegNum reg, const GenAddrMode& am, unsigned offset, int imm)
{
    Emitter& emit = *GetEmitter();
    ...
    {
        emit.Ins_R_ARX_I(ins, attr, reg, am.Base(), am.Index(), am.Scale(), am.Disp() + offset, imm);
    }
}
...
void CodeGen::inst_AM_R(instruction ins, emitAttr attr, RegNum reg, const GenAddrMode& addrMode, unsigned offset)
{
    ...
    if (addrMode.IsLcl())
    {
        emit.Ins_S_R(ins, attr, reg, GetStackAddrMode(addrMode.LclNum(), addrMode.Disp(offset)));
    }
    else
    {
...
void CodeGen::StoreSIMD12(GenTree* store, const GenAddrMode& dst, GenTree* value)
{
    ...
    {
        RegNum tmpReg = store->GetSingleTempReg();
        GetEmitter()->Ins_R_R(INS_movhlps, EA_16BYTE, tmpReg, valueReg);
        inst_AM_R(INS_movss, EA_4BYTE, tmpReg, dst, 8);
    }
}
...
void CodeGen::LoadSIMD12(GenTree* load)
{
    ...
    else
    {
        ...
        inst_R_AM(INS_movss, EA_4BYTE, tmpReg, src, 8);
        GetEmitter()->Ins_R_R(INS_movlhps, EA_16BYTE, dstReg, tmpReg);
    }
}
...
void CodeGen::PushSIMD12(GenTree* store, RegNum valueReg) const
{
    ...
    emit.Ins_AR_R(INS_movsd, EA_8BYTE, valueReg, REG_SPBASE, 0);
    ...
    else
    {
        ...
        emit.Ins_R_R(INS_movhlps, EA_16BYTE, tmpReg, valueReg);
        emit.Ins_AR_R(INS_movss, EA_4BYTE, tmpReg, REG_SPBASE, 8);
    }
}
...
void CodeGen::GenVectorUpperUnspill(GenTreeUnOp* node)
{
    ...
    if (srcReg != REG_NA)
    {
        GetEmitter()->Ins_R_R_R_I(INS_vinsertf128, EA_32BYTE, dstReg, dstReg, srcReg, 1);
    }
    else
    {
        LclVarDsc* lcl = op1->AsLclLoad()->GetLcl();
        ...
        GetEmitter()->Ins_R_R_S_I(INS_vinsertf128, EA_32BYTE, dstReg, dstReg, GetStackAddrMode(lcl, 16), 1);
    }
}
...
void CodeGen::PrologPreserveCalleeSavedFloatRegs(unsigned lclFrameSize)
{
    ...
    for (regNumber reg = REG_FLT_CALLEE_SAVED_FIRST; regMask != RBM_NONE; reg = REG_NEXT(reg))
    {
        ...
        {
            GetEmitter()->Ins_AR_R(INS_movaps, EA_16BYTE, reg, REG_RSP, offset);
...
void CodeGen::genRestoreCalleeSavedFltRegs(unsigned lclFrameSize)
{
    ...
    for (RegNum reg = REG_FLT_CALLEE_SAVED_FIRST; regMask != RBM_NONE; reg = REG_NEXT(reg))
    {
        ...
        if ((regBit & regMask) != RBM_NONE)
        {
            GetEmitter()->Ins_R_AR(INS_movaps, EA_16BYTE, reg, regBase, offset);
            regMask &= ~regBit;
            offset -= XMM_REGSIZE_BYTES;
        }
    }
...
void CodeGen::PrologBlockInitLocals(int untrLclLo, int untrLclHi, RegNum initReg, bool* pInitRegZeroed)
{
    ...
    Emitter& emit        = *GetEmitter();
    RegNum   frameReg    = genFramePointerReg();
    RegNum   zeroReg     = REG_NA;
    ...
    if (blkSize < minSimdSize)
    {
        ...
        {
            emit.Ins_AR_R(INS_mov, EA_PTRSIZE, zeroReg, frameReg, untrLclLo + i);
        }
#ifdef TARGET_AMD64
        assert((i == blkSize) || (i + 4 == blkSize));
        if (i != blkSize)
        {
            emit.Ins_AR_R(INS_mov, EA_4BYTE, zeroReg, frameReg, untrLclLo + i);
            i += 4;
        }
#endif
        assert(i == blkSize);
    ...
    else
    {
        ...
        if (untrLclHi != alignedLclHi)
        {
            ...
            {
                emit.Ins_AR_R(INS_mov, EA_PTRSIZE, zeroReg, frameReg, alignedLclHi + i);
            }
#ifdef TARGET_AMD64
            assert((i == alignmentHiBlkSize) || (i + 4 == alignmentHiBlkSize));
            if (i != alignmentHiBlkSize)
            {
                emit.Ins_AR_R(INS_mov, EA_4BYTE, zeroReg, frameReg, alignedLclHi + i);
                i += 4;
            }
#endif
...
void CodeGen::PrologZeroRegs(regMaskTP initRegs)
{
    ...
    for (RegNum reg = REG_INT_FIRST; reg <= REG_INT_LAST; reg = REG_NEXT(reg))
    {
        ...
        emit.Ins_R_R(INS_xor, EA_4BYTE, reg, reg);
    }
    ...
    for (RegNum reg = REG_FP_FIRST; reg <= REG_FP_LAST; reg = REG_NEXT(reg))
    {
        ...
        emit.Ins_R_R(INS_xorps, EA_16BYTE, reg, reg);
    }
}
...
void CodeGen::PrologInitVarargsStackParamsBaseOffset()
{
    ...
    emit.Ins_R_S(INS_mov, EA_4BYTE, REG_EAX, GetStackAddrMode(varargsHandleLclNum, 0));
    emit.Ins_R_AR(INS_mov, EA_4BYTE, REG_EAX, REG_EAX, 0);
    ...
    emit.Ins_R_ARX(INS_lea, EA_4BYTE, REG_EAX, genFramePointerReg(), REG_EAX, 1, offset);

    LclVarDsc* lcl = compiler->lvaVarargsBaseOfStkLcl;

    if (lcl->IsInReg())
    {
        emit.emitIns_Mov(INS_mov, EA_4BYTE, lcl->GetRegNum(), REG_EAX, /* canSkip */ true);
    }
    else
    {
        emit.Ins_S_R(INS_mov, EA_4BYTE, REG_EAX, GetStackAddrMode(lcl, 0));
    }
}
#endif // TARGET_X86
...
void CodeGen::genFuncletProlog(BasicBlock* block)
{
    ...
    emit.Ins_R(INS_push, EA_GCREF, REG_FPBASE);
    ...
    emit.Ins_R_AR(INS_mov, EA_PTRSIZE, REG_FPBASE, REG_ARG_0, genFuncletInfo.fiPSP_slot_InitialSP_offset);
    emit.Ins_AR_R(INS_mov, EA_PTRSIZE, REG_FPBASE, REG_SPBASE, genFuncletInfo.fiPSP_slot_InitialSP_offset);

    if (genFuncletInfo.fiFunction_InitialSP_to_FP_delta != 0)
    {
        emit.Ins_R_AR(INS_lea, EA_PTRSIZE, REG_FPBASE, REG_FPBASE, genFuncletInfo.fiFunction_InitialSP_to_FP_delta);
    }
}
...
void CodeGen::genFuncletEpilog()
{
    ...
    emit.Ins_R_I(INS_add, EA_PTRSIZE, REG_SPBASE, genFuncletInfo.fiSpDelta);
    genPopCalleeSavedRegisters();
    emit.Ins_R(INS_pop, EA_PTRSIZE, REG_EBP);
    emit.emitIns(INS_ret);
}
...
void CodeGen::genFnEpilog(BasicBlock* block)
{
    ...
    if (!removeEbpFrame)
    {
        ...
        if (lclFrameSize != 0)
        {
            ...
            if ((lclFrameSize == REGSIZE_BYTES) && !compiler->compJmpOpUsed)
            {
                ...
                emit.Ins_R(INS_pop, EA_4BYTE, REG_ECX);
            }
            else
#endif // TARGET_X86
            {
                emit.Ins_R_I(INS_add, EA_PTRSIZE, REG_RSP, static_cast<int>(lclFrameSize));
            }
        }
        ...
        if (isFramePointerUsed())
        {
            emit.Ins_R(INS_pop, EA_8BYTE, REG_RBP);
        }
#endif // TARGET_AMD64
        ...
        if (compiler->opts.IsOSR())
        {
            ...
            // need to change.
            emit.Ins_R_I(INS_add, EA_PTRSIZE, REG_SPBASE, originalFrameSize);
            emit.Ins_R(INS_pop, EA_PTRSIZE, REG_EBP);
        }
    }
    else
    {
        ...
        if (doDoubleAlign())
        {
            ...
            emit.Ins_R_I(INS_add, EA_4BYTE, REG_SPBASE, static_cast<int32_t>(lclFrameSize));
...
#endif // DOUBLE_ALIGN
        {
            ...
            if (needLea)
            {
                ...
                emit.Ins_R_AR(INS_lea, EA_PTRSIZE, REG_SPBASE, REG_FPBASE, -offset);
            }
        }
        ...
        if (compiler->opts.IsOSR())
        {
            ...
            // If we ever allow the original method to have localloc this will
            // need to change.
            emit.Ins_R_I(INS_add, EA_PTRSIZE, REG_SPBASE, originalFrameSize + TARGET_POINTER_SIZE);
        }
#endif
        ...
        if (needMovEspEbp)
        {
            emit.emitIns_Mov(INS_mov, EA_4BYTE, REG_ESP, REG_EBP, /* canSkip */ false);
        }
#endif
    ...
    {
        emit.Ins_I(INS_ret, EA_4BYTE, paramsStackSize);
    }
#endif // TARGET_X86
}
...
void CodeGen::inst_RM(instruction ins, emitAttr attr, GenTree* src)
{
    ...
    {
        emit.Ins_S(ins, attr, s);
    }
    else
    {
        emit.Ins_A(ins, attr, src->AsIndir()->GetAddr());
    }
}
...
void CodeGen::inst_R_RM(instruction ins, emitAttr attr, regNumber reg, GenTree* rm)
{
    ...
    if (rm->isUsedFromReg())
    {
        emit.Ins_R_R(ins, attr, reg, rm->GetRegNum());
    }
    ...
    {
        emit.Ins_R_A(ins, attr, reg, rm->AsIndLoad()->GetAddr());
    }
}
...
void CodeGen::inst_Cmp_RM_RM(instruction ins, emitAttr attr, GenTree* op1, GenTree* op2)
{
    ...
    else
    {
        ...
        emit.Ins_R_R(ins, attr, op1->GetRegNum(), op2->GetRegNum());
    ...
    if (IsLocalMemoryOperand(memOp, &s))
    {
        if (memOp == op2)
        {
            emit.Ins_R_S(ins, attr, regOp->GetRegNum(), s);
        }
        ...
        {
            emit.Ins_S_R(ins, attr, regOp->GetRegNum(), s);
        }
    }
    else
    {
        ...
        if (memOp == op2)
        {
            emit.Ins_R_A(ins, attr, regOp->GetRegNum(), addr);
        }
        else if (immOp != nullptr)
        {
            emit.Ins_A_I(ins, attr, addr, immOp->AsIntCon()->GetInt32Value());
        }
        else
        {
            emit.Ins_A_R(ins, attr, addr, regOp->GetRegNum());
        }
    }
}
...
void CodeGen::genJumpToThrowHlpBlk(emitJumpKind condition, ThrowHelperKind throwKind, BasicBlock* throwBlock)
{
    ...
    if (useThrowHelperBlocks)
    {
        ...
        emit.Ins_J(JumpKindToJcc(condition), throwBlock->emitLabel);
    }
    else
    {
        insGroup* label = GetEmitter()->CreateTempLabel();
        emit.Ins_J(JumpKindToJcc(ReverseJumpKind(condition)), label);
