        ...
        if (offset != 0)
        {
            ...
            if (emitIns_valid_imm_for_add(offset, INS_FLAGS_DONT_CARE))
            {
                if (lsl > 0)
                {
                    ...
                    emitIns_R_R_R_I(INS_add, leaBasePartialAddrAttr, tmpReg, base->GetRegNum(), index->GetRegNum(), lsl,
            ...
            else // large offset
            {
                ...
                emitIns_R_R_R_I(ins, attr, dataReg, tmpReg, index->GetRegNum(), lsl, INS_FLAGS_DONT_CARE, INS_OPTS_LSL);
            }
        }
        else // (offset == 0)
        {
